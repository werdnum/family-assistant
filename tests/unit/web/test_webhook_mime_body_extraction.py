...
def test_populate_missing_stripped_text_prefers_existing_body_plain() -> None:
    html = "<p>Noisier raw MIME HTML body</p>"
    raw_mime = (
        b"MIME-Version: 1.0\r\n"
        b"Content-Type: text/html; charset=utf-8\r\n"
        b"\r\n" + html.encode()
    )
    form_data: dict[str, object] = {"body-plain": "Mailgun parsed body"}

    webhooks._populate_missing_body_fields_from_mime(form_data, raw_mime)

    assert form_data["body-plain"] == "Mailgun parsed body"
    assert form_data["stripped-text"] == "Mailgun parsed body"
    assert form_data["body-html"] == html
    assert form_data["stripped-html"] == html
