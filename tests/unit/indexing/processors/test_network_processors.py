...
def mock_tool_execution_context() -> MagicMock:
    ...
    return MagicMock(spec=["db_context", "enqueue_task", "llm_interface", "embedding_generator"])


@pytest.fixture
def default_config() -> WebFetcherProcessorConfig:
    """Default configuration for the processor."""
    return WebFetcherProcessorConfig(process_embedding_types=["extracted_link", "raw_url"])
...
async def test_fetch_image_content_success(
    ...
    assert result_item.metadata["original_filename"] == "image.png" # Based on URL parsing
...
async def test_multiple_items_processing(
    ...
    scrape_map = {
        ...
        url_img: ScrapeResult(type="image", final_url=url_img, content_bytes=b"img", mime_type="image/jpeg"),
        url_err: ScrapeResult(type="error", final_url=url_err, message="Failed fetch"),
    }
    mock_scraper = MockScraper(url_map=scrape_map)
    processor = WebFetcherProcessor(scraper=mock_scraper, config=default_config)

    item_md = IndexableContent(content=url_md, embedding_type="extracted_link")
    item_img = IndexableContent(content=url_img, embedding_type="raw_url")
    item_err = IndexableContent(content=url_err, embedding_type="extracted_link")
    item_pass_type = IndexableContent(content="http://example.com/pass", embedding_type="other_type")
    item_pass_content = IndexableContent(content="not a url", embedding_type="extracted_link")
    ...
    mock_initial_ref = IndexableContent(content="initial", embedding_type="initial_type")
    ...
    for r_item in results:
        if r_item.embedding_type == "fetched_content_markdown" and r_item.metadata.get("original_url") == url_md:
            fetched_md_count += 1
            assert r_item.content == "MD Content"
        elif r_item.embedding_type == "fetched_content_binary" and r_item.metadata.get("original_url") == url_img:
            fetched_img_count += 1
...
async def test_cleanup_temp_files_file_externally_deleted(
    ...
    scrape_map = {
        url: ScrapeResult(type="image", final_url=url, content_bytes=b"gif_data", mime_type="image/gif")
    }
    mock_scraper = MockScraper(url_map=scrape_map)
    processor = WebFetcherProcessor(scraper=mock_scraper, config=default_config)

    input_item = IndexableContent(content=url, embedding_type="extracted_link")
    results = await processor.process([input_item], mock_document, input_item, mock_tool_execution_context)
...
def test_del_cleanup_fallback(
    ...
    if os.path.exists(fake_temp_file_path):
        ...
        pytest.fail(f"__del__ fallback cleanup did not remove the temp file: {fake_temp_file_path}")
