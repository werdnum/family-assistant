...
class TestHomeAssistantValidation:
    ...
    def mock_client(self) -> Mock:
        ...
        mock_states = [
            ...
            Mock(entity_id="person.teija_smith"),
    ...
    async def test_shortened_person_entity(
        ...
        assert error.suggestion == "Did you mean 'person.andrew_garrett'?"
        assert error.similar_values is not None
        assert "person.andrew_garrett" in error.similar_values
    ...
    async def test_invalid_entity_format(self, ha_source: HomeAssistantSource) -> None:
        ...
        for entity_id, error_type in test_cases:
            ...
            if error_type == "format":
                ...
                assert (
                    "person.andrew_garrett" in error.suggestion
    ...
    async def test_nonexistent_entity(self, ha_source: HomeAssistantSource) -> None:
        ...
        assert "person.andrew_garrett" in error.similar_values
        assert "person.teija_smith" in error.similar_values
    ...
    @pytest.mark.asyncio
    async def test_teija_entity_suggestion(
        self, ha_source: HomeAssistantSource
    ) -> None:
        """Test validation provides suggestion for Teija entity."""
        result = await ha_source.validate_match_conditions({
            "entity_id": "person.teija"
        })
        assert result.valid is False
        assert len(result.errors) == 1
        error = result.errors[0]
        assert error.suggestion == "Did you mean 'person.teija_smith'?"
