...
class TestHashingWordEmbeddingGenerator:
    ...
    async def test_quote_similarity(self, default_generator: HashingWordEmbeddingGenerator):
        ...
        if len(quote) < len(source_text):
             assert not math.isclose(dist_source_quote, 0.0, abs_tol=1e-9), "Source and its shorter quote should not be identical"
    ...
    async def test_batch_processing(self, default_generator: HashingWordEmbeddingGenerator):
    ...
    def test_invalid_initialization(self):
        ...
        with pytest.raises(ValueError, match="Dimensionality must be a positive integer"):
            HashingWordEmbeddingGenerator(dimensionality=0)
        with pytest.raises(ValueError, match="Dimensionality must be a positive integer"):
            HashingWordEmbeddingGenerator(dimensionality=-5)

    async def test_empty_input_list(self, default_generator: HashingWordEmbeddingGenerator):
