...
class TestEventConditionEvaluator:
    ...
    def test_simple_boolean_condition(self, evaluator: EventConditionEvaluator) -> None:
        """Test simple boolean return."""
        script = "True"
        result = evaluator.evaluate_condition(script, {})
    ...
    def test_event_data_access(self, evaluator: EventConditionEvaluator) -> None:
        """Test accessing event data."""
        script = "event.get('entity_id') == 'person.test'"
        event_data = {"entity_id": "person.test"}
        result = evaluator.evaluate_condition(script, event_data)
        assert result is True
    ...
    def test_any_state_change(self, evaluator: EventConditionEvaluator) -> None:
    ...
    def test_complex_conditions(self, evaluator: EventConditionEvaluator) -> None:
    ...
    def test_non_boolean_return(self, evaluator: EventConditionEvaluator) -> None:
        """Test error on non-boolean return."""
        script = "'not a boolean'"
        with pytest.raises(ScriptExecutionError) as exc_info:
            evaluator.evaluate_condition(script, {})
        assert "must return boolean" in str(exc_info.value)

    def test_syntax_error(self, evaluator: EventConditionEvaluator) -> None:
        """Test syntax error handling."""
        script = "True ("  # Invalid syntax
        with pytest.raises(ScriptSyntaxError):
            evaluator.evaluate_condition(script, {})

    def test_runtime_error(self, evaluator: EventConditionEvaluator) -> None:
        """Test runtime error handling."""
        script = "undefined_variable"
        with pytest.raises(ScriptExecutionError):
            evaluator.evaluate_condition(script, {})

    def test_no_tool_access(self, evaluator: EventConditionEvaluator) -> None:
        """Test that tools are not accessible."""
        script = "tools_list()"
        with pytest.raises(ScriptExecutionError):
            evaluator.evaluate_condition(script, {})

    def test_no_print_function(self, evaluator: EventConditionEvaluator) -> None:
...
class TestEventConditionValidator:
    ...
    def test_valid_script(self, validator: EventConditionValidator) -> None:
        """Test validating a valid script."""
        script = "event.get('state') == 'on'"
        is_valid, error = validator.validate_script(script)
        assert is_valid is True
        assert error is None

    def test_syntax_error_validation(self, validator: EventConditionValidator) -> None:
    ...
    def test_non_boolean_validation(self, validator: EventConditionValidator) -> None:
    ...
    def test_script_size_limit(self, validator: EventConditionValidator) -> None:
        """Test script size validation."""
        # Create a script larger than 10KB
        large_script = "# " + "x" * 10240 + "\nTrue"
        is_valid, error = validator.validate_script(large_script)
        assert is_valid is False
