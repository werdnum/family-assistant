...
from family_assistant.embeddings import (
    ...
    EmbeddingGenerator, # Keep if used by type hints, else can remove if only MockEmbeddingGenerator is used.
)  # Import protocol
...
from family_assistant.indexing.processors.dispatch_processors import EmbeddingDispatchProcessor # Added
...
async def test_handle_embed_and_store_batch(
    exec_context: ToolExecutionContext, payload: Dict[str, Any]
):
    logger.info(f"Test task handler 'test_handle_embed_and_store_batch' received payload: {payload}")
    ...
    if not texts_to_embed:
        logger.warning(f"No texts to embed for document {document_id} in 'embed_and_store_batch'.")
        return
    ...
    for i, vector in enumerate(embedding_result.embeddings):
        meta = embedding_metadata_list[i] # This meta contains embedding_type, chunk_index, original_content_metadata
    ...
    logger.info(f"Stored {len(texts_to_embed)} embeddings for document {document_id} via test_handle_embed_and_store_batch.")
...
async def test_document_indexing_and_query_e2e(
    ...
    dispatch_processor = EmbeddingDispatchProcessor(
        dispatchable_embedding_types={"title", "content_chunk"},
        embedding_generator_model_override=None, # Uses default/global generator
    )
    ...
    logger.info(
        f"DocumentIndexer initialized with IndexingPipeline."
    )
    ...
    worker.register_task_handler(
        "embed_and_store_batch", test_handle_embed_and_store_batch # Register test handler
    )
    logger.info(
