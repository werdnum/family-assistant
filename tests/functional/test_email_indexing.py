...
async def _ingest_and_index_email(
    ...
    # Find the embed_and_store_batch tasks generated by the pipeline for this document
    embed_task_ids = set()
    async with DatabaseContext(engine=engine) as db:
        select_embed_tasks = select(tasks_table.c.task_id).where(
            tasks_table.c.task_type == "embed_and_store_batch",
            # Assuming task ID format is predictable or filter by payload if needed
            tasks_table.c.task_id.like(f"embed_batch_{email_db_id}_%")
        )
        results = await db.fetch_all(select_embed_tasks)
        embed_task_ids = {row["task_id"] for row in results}

    if embed_task_ids:
        logger.info(f"Helper: Waiting for embedding tasks: {embed_task_ids}")
        await wait_for_tasks_to_complete(
            engine, task_ids=embed_task_ids, timeout_seconds=task_timeout
        )
        logger.info("Helper: Embedding tasks reported as complete.")
    else:
        # It's possible a document results in no embeddable content after processing
        logger.warning(f"Helper: No embed_and_store_batch tasks found for document {email_db_id}. Skipping second wait.")
...
async def test_vector_ranking(pg_vector_db_engine):
    ...
    try:
        ...
        await wait_for_tasks_to_complete(
            pg_vector_db_engine, task_ids=all_task_ids, timeout_seconds=20.0
        )
        ...
        # --- Assert ---
        assert (
            query_results is not None and len(query_results) >= 3
        ...
        try:
            ...
            pytest.fail(
                f"One or more test emails not found in results: {results_ordered}"
            )

        # Assert the order based on distance (lower index means closer/better rank)
        assert (
            idx1 < idx2
        ), f"Closest email ({email1_msg_id}) was not ranked higher than medium ({email2_msg_id})"
        assert (
            idx2 < idx3
        ), f"Medium email ({email2_msg_id}) was not ranked higher than farthest ({email3_msg_id})"
...
async def test_metadata_filtering(pg_vector_db_engine):
    ...
    try:
        ...
        assert (
            len(query_results) > 0
        ), f"Expected at least 1 result matching filter, got {len(query_results)}"
        ...
        assert (
            email1_msg_id not in source_ids_returned
...
async def test_keyword_filtering(pg_vector_db_engine):
    ...
    try:
        ...
        assert (
            found_result.get("source_id") == email2_msg_id
        ), f"Top result should be the one matching keyword '{keyword}'"
        assert (
            keyword in found_result.get("embedding_source_content", "").lower()
        ), "Keyword not found in top result content"
        assert "rrf_score" in found_result, "Result missing 'rrf_score'"
        assert (
            found_result.get("fts_score", 0) > 0
        ), "Keyword match should have a positive FTS score"
        ...
        if non_matching_present:
            ...
            assert (
                rank_match < rank_non_match
            ), "Keyword match should be ranked higher than non-match"
            logger.info(
