...
async def _ingest_and_index_email(
    ...
    # Add a second wait: Wait for *any* remaining tasks (like embed_and_store_batch)
    # generated by the pipeline to complete.
    logger.info("Helper: Waiting for any subsequent pipeline tasks to complete...")
    await wait_for_tasks_to_complete(
        engine, timeout_seconds=task_timeout
    ) # task_ids=None waits for all
    logger.info("Helper: All subsequent tasks reported as complete.")
