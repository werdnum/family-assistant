...
def get_radicale_client(radicale_server_details: tuple[str, str, str, str]) -> caldav.DAVClient:
...
async def get_event_by_summary_from_radicale(
    ...
    target_calendar = next((cal for cal in calendars if cal.name == calendar_name), None)
    ...
    for event_obj in events:  # event_obj is caldav.objects.Event
        ...
        try:
            ...
            if vevent and hasattr(vevent, "summary") and vevent.summary.value == event_summary:
                return event_obj
...
async def test_add_event_and_verify_in_system_prompt(
    ...
    def add_event_matcher(kwargs: MatcherArgs) -> bool:
        ...
        return f"schedule {event_summary.lower()}" in last_text and kwargs.get("tools") is not None
    ...
    llm_client: LLMInterface = RuleBasedMockLLMClient(rules=[(add_event_matcher, add_event_response)])
    ...
    dummy_prompts = {"system_prompt": "System: {calendar_today_tomorrow}\nFuture: {calendar_next_two_weeks}"}
    ...
    radicale_event = await get_event_by_summary_from_radicale(radicale_server, RADICALE_TEST_CALENDAR_NAME, event_summary)
    ...
    system_prompt_context_parts = await processing_service._aggregate_context_from_providers(db_context, "test", TEST_CHAT_ID, TEST_USER_NAME)
    system_prompt_str = dummy_prompts["system_prompt"].format(**system_prompt_context_parts)
    ...
    assert event_summary in system_prompt_str, "Event summary not found in system prompt."
    assert expected_time_str_in_prompt in system_prompt_str, f"Expected time '{expected_time_str_in_prompt}' not found in system prompt."
...
async def test_modify_event(
    ...
    original_start_dt = day_after_tomorrow.replace(hour=14, minute=0, second=0, microsecond=0)
    ...
    principal = await asyncio.to_thread(client.principal)
    calendars = await asyncio.to_thread(principal.calendars)
    target_calendar = next((cal for cal in calendars if cal.name == RADICALE_TEST_CALENDAR_NAME), None)
    assert target_calendar is not None, f"Test calendar '{RADICALE_TEST_CALENDAR_NAME}' not found."
...
END:VEVENT
END:VCALENDAR"""
    created_event_radicale = await asyncio.to_thread(target_calendar.add_event, vcal=event_vcal)
    event_uid = created_event_radicale.vobject_instance.vevent.uid.value
    logger.info(f"Directly created event '{original_summary}' with UID {event_uid} in Radicale.")
    ...
    llm_client: LLMInterface = RuleBasedMockLLMClient(rules=[(modify_event_matcher, modify_event_response)])

    # --- Setup ProcessingService (similar to add_event test) ---
    test_calendar_config = {
        "caldav": {"username": r_user, "password": r_pass, "calendar_urls": [test_calendar_direct_url]},
        "ical": {"urls": []}
    }
    dummy_prompts = {"system_prompt": "System: {calendar_today_tomorrow}\nFuture: {calendar_next_two_weeks}"}
    local_provider = LocalToolsProvider(definitions=local_tools_definition, implementations=local_tool_implementations, calendar_config=test_calendar_config)
    mcp_provider = MCPToolsProvider(mcp_server_configs={})
    composite_provider = CompositeToolsProvider(providers=[local_provider, mcp_provider])
    await composite_provider.get_tool_definitions()
    ...
    calendar_context_provider = CalendarContextProvider(get_db_context_func=get_test_db_context_func, calendar_config=test_calendar_config, prompts=dummy_prompts, timezone_str=TEST_TIMEZONE_STR)
    service_config = ProcessingServiceConfig(id="test_cal_mod_profile", prompts=dummy_prompts, calendar_config=test_calendar_config, timezone_str=TEST_TIMEZONE_STR, max_history_messages=5, history_max_age_hours=24, tools_config={"confirmation_required": []}, delegation_security_level="unrestricted")
    processing_service = ProcessingService(llm_client=llm_client, tools_provider=composite_provider, context_providers=[calendar_context_provider], service_config=service_config, server_url=None, app_config={})
    ...
    async with DatabaseContext(engine=pg_vector_db_engine) as db_context:
        final_reply_modify, _, _, error_modify = await processing_service.handle_chat_interaction(
            ...
            trigger_interface_message_id="msg_mod", user_name=TEST_USER_NAME,
        )

    assert error_modify is None, f"Error during modify chat interaction: {error_modify}"
    assert final_reply_modify and f"OK. Event '{modified_summary}' updated." in final_reply_modify  # Check tool's success message

    # --- Verify Event in Radicale ---
    modified_radicale_event = await get_event_by_summary_from_radicale(radicale_server, RADICALE_TEST_CALENDAR_NAME, modified_summary)
    assert modified_radicale_event is not None, f"Event '{modified_summary}' not found in Radicale after modification."
    
    # Verify original summary event is gone
    original_radicale_event_after_modify = await get_event_by_summary_from_radicale(radicale_server, RADICALE_TEST_CALENDAR_NAME, original_summary)
    assert original_radicale_event_after_modify is None, f"Event with original summary '{original_summary}' still found in Radicale after modification."

    # --- Verify Modified Event in System Prompt ---
    await asyncio.sleep(0.5)
    system_prompt_context_parts_mod = await processing_service._aggregate_context_from_providers(db_context, "test", TEST_CHAT_ID, TEST_USER_NAME)  # type: ignore
    system_prompt_str_mod = dummy_prompts["system_prompt"].format(**system_prompt_context_parts_mod)
    logger.info(f"Generated system prompt after modification:\n{system_prompt_str_mod}")

    formatted_day_after_tomorrow = format_datetime_or_date(day_after_tomorrow.date(), TEST_TIMEZONE_STR)
    expected_time_str_in_prompt_mod = f"{formatted_day_after_tomorrow} 15:00"  # 3 PM
    assert modified_summary in system_prompt_str_mod, "Modified event summary not found in system prompt."
    assert expected_time_str_in_prompt_mod in system_prompt_str_mod, f"Expected modified time '{expected_time_str_in_prompt_mod}' not found."
    assert original_summary not in system_prompt_str_mod, "Original event summary still found in system prompt after modification."
...
async def test_delete_event(
    ...
    local_tz = ZoneInfo(TEST_TIMEZONE_STR)
    event_start_dt = (datetime.now(local_tz) + timedelta(days=3)).replace(hour=11, minute=0, second=0, microsecond=0)
...
END:VEVENT
END:VCALENDAR"""
    created_event_radicale_del = await asyncio.to_thread(target_calendar.add_event, vcal=event_vcal_del)
    event_uid_del = created_event_radicale_del.vobject_instance.vevent.uid.value
    logger.info(f"Directly created event '{event_to_delete_summary}' with UID {event_uid_del} for deletion test.")
    ...
    llm_client: LLMInterface = RuleBasedMockLLMClient(rules=[(delete_event_matcher, delete_event_response)])
    ...
    dummy_prompts = {"system_prompt": "System: {calendar_today_tomorrow}\nFuture: {calendar_next_two_weeks}"}  # type: ignore
    local_provider = LocalToolsProvider(definitions=local_tools_definition, implementations=local_tool_implementations, calendar_config=test_calendar_config)
    mcp_provider = MCPToolsProvider(mcp_server_configs={})
    composite_provider = CompositeToolsProvider(providers=[local_provider, mcp_provider])
    await composite_provider.get_tool_definitions()
    ...
    calendar_context_provider = CalendarContextProvider(get_db_context_func=get_test_db_context_func, calendar_config=test_calendar_config, prompts=dummy_prompts, timezone_str=TEST_TIMEZONE_STR)  # type: ignore
    service_config = ProcessingServiceConfig(id="test_cal_del_profile", prompts=dummy_prompts, calendar_config=test_calendar_config, timezone_str=TEST_TIMEZONE_STR, max_history_messages=5, history_max_age_hours=24, tools_config={"confirmation_required": []}, delegation_security_level="unrestricted")  # type: ignore
    processing_service = ProcessingService(llm_client=llm_client, tools_provider=composite_provider, context_providers=[calendar_context_provider], service_config=service_config, server_url=None, app_config={})

    # --- Simulate User Interaction to Delete ---
    user_message_delete = f"Please delete event {event_to_delete_summary}."
    async with DatabaseContext(engine=pg_vector_db_engine) as db_context:
        final_reply_delete, _, _, error_delete = await processing_service.handle_chat_interaction(
            db_context=db_context, chat_interface=MagicMock(), new_task_event=asyncio.Event(),
            interface_type="test", conversation_id=TEST_CHAT_ID,
            trigger_content_parts=[{"type": "text", "text": user_message_delete}],
            trigger_interface_message_id="msg_del", user_name=TEST_USER_NAME,
        )

    assert error_delete is None, f"Error during delete chat interaction: {error_delete}"
    assert final_reply_delete and f"OK. Event '{event_to_delete_summary}' deleted." in final_reply_delete

    # --- Verify Event in Radicale ---
    deleted_radicale_event = await get_event_by_summary_from_radicale(radicale_server, RADICALE_TEST_CALENDAR_NAME, event_to_delete_summary)
    assert deleted_radicale_event is None, f"Event '{event_to_delete_summary}' still found in Radicale after deletion."

    # --- Verify Event NOT in System Prompt ---
    await asyncio.sleep(0.5)
    system_prompt_context_parts_del = await processing_service._aggregate_context_from_providers(db_context, "test", TEST_CHAT_ID, TEST_USER_NAME)  # type: ignore
    system_prompt_str_del = dummy_prompts["system_prompt"].format(**system_prompt_context_parts_del)  # type: ignore
    logger.info(f"Generated system prompt after deletion:\n{system_prompt_str_del}")

    assert event_to_delete_summary not in system_prompt_str_del, "Deleted event summary still found in system prompt."
...
async def test_search_events(
    ...
    for summ, st, en in [(event1_summary, event1_start, event1_end), (event2_summary, event2_start, event2_end)]:
...
END:VCALENDAR"""
    ...
    logger.info(f"Directly created '{event1_summary}' and '{event2_summary}' for search test.")

    # --- LLM Rules ---
    tool_call_id_search = f"call_search_{uuid.uuid4()}"
    search_query_text = "events for day after tomorrow plus two"  # A bit vague to test search
            ...
            ToolCallItem(
                ...
                function=ToolCallFunction(
                    ...
                    arguments=json.dumps({
                        ...
                        "end_date_str": (search_day + timedelta(days=1)).strftime('%Y-%m-%d'),  # Search for one day
                    }),
                ),
            )
        ],
    ...
    def present_search_results_matcher(kwargs: MatcherArgs) -> bool:
        ...
        prev_msg = messages[-2]
        return (
            prev_msg.get("role") == "tool" and
            prev_msg.get("tool_call_id") == tool_call_id_search and
            event1_summary.lower() in prev_msg.get("content", "").lower() and  # Check if tool output contains expected event
            last_msg.get("role") == "user"  # This is actually the system prompt for the LLM to generate the *next* response
        )
    ...
    llm_client: LLMInterface = RuleBasedMockLLMClient(rules=[
    ...
    dummy_prompts: dict[str, Any] = {"system_prompt": "System: {calendar_today_tomorrow}\nFuture: {calendar_next_two_weeks}"}
    local_provider = LocalToolsProvider(definitions=local_tools_definition, implementations=local_tool_implementations, calendar_config=test_calendar_config)
    mcp_provider = MCPToolsProvider(mcp_server_configs={})
    composite_provider = CompositeToolsProvider(providers=[local_provider, mcp_provider])
    await composite_provider.get_tool_definitions()
    ...
    calendar_context_provider = CalendarContextProvider(get_db_context_func=get_test_db_context_func, calendar_config=test_calendar_config, prompts=dummy_prompts, timezone_str=TEST_TIMEZONE_STR)
    service_config = ProcessingServiceConfig(id="test_cal_search_profile", prompts=dummy_prompts, calendar_config=test_calendar_config, timezone_str=TEST_TIMEZONE_STR, max_history_messages=5, history_max_age_hours=24, tools_config={"confirmation_required": []}, delegation_security_level="unrestricted")
    processing_service = ProcessingService(llm_client=llm_client, tools_provider=composite_provider, context_providers=[calendar_context_provider], service_config=service_config, server_url=None, app_config={})

    # --- Simulate User Interaction ---
    user_message_search = f"What are my events for {search_query_text}?"
    async with DatabaseContext(engine=pg_vector_db_engine) as db_context:
        final_reply_search, _, _, error_search = await processing_service.handle_chat_interaction(
            db_context=db_context, chat_interface=MagicMock(), new_task_event=asyncio.Event(),
            interface_type="test", conversation_id=TEST_CHAT_ID,
            trigger_content_parts=[{"type": "text", "text": user_message_search}],
            trigger_interface_message_id="msg_search", user_name=TEST_USER_NAME,
        )

    assert error_search is None, f"Error during search chat interaction: {error_search}"
    assert final_reply_search is not None
    assert event1_summary in final_reply_search, "Event 1 summary not in LLM's final reply."
    assert event2_summary in final_reply_search, "Event 2 summary not in LLM's final reply."
