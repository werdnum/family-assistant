...
async def test_schedule_and_execute_callback(
    ...
    test_chat_id = get_unique_test_chat_id()
    logger.info(
        f"\n--- Running Callback Test ({test_run_id}) with chat_id={test_chat_id} ---"
    )
    ...
    async with DatabaseContext(engine=db_engine) as db_context:
        ...
        schedule_final_text_reply = result.text_reply
...
@pytest.mark.asyncio
async def test_modify_pending_action(
    db_engine: AsyncEngine,
    task_worker_manager: Callable[..., tuple[TaskWorker, asyncio.Event, asyncio.Event]],
    mock_clock: MockClock,
) -> None:
    ...
    5. Mock LLM calls modify_pending_action tool.
    ...
    test_chat_id = get_unique_test_chat_id()
    logger.info(
        f"\n--- Running Modify Callback Test ({test_run_id}) with chat_id={test_chat_id} ---"
    )
    ...
    # We need to capture it to use in the modify_pending_action tool call.
    ...
    # In a real scenario, the LLM would get this from list_pending_actions.
            ...
            ToolCallItem(
                ...
                function=ToolCallFunction(
                    ...
                    arguments=json.dumps({
                        ...
                        "new_schedule_time": modified_callback_time_iso,
    ...
    # Update the placeholder in the LLM rule's arguments for modify_pending_action
...
async def test_cancel_pending_action(
    ...
    5. Mock LLM calls cancel_pending_action tool.
    ...
    test_chat_id = get_unique_test_chat_id()
    logger.info(
        f"\n--- Running Cancel Callback Test ({test_run_id}) with chat_id={test_chat_id} ---"
    )
...
async def test_schedule_reminder_with_follow_up(
    ...
    test_chat_id = get_unique_test_chat_id()
    logger.info(
        f"\n--- Running Reminder with Follow-up Test ({test_run_id}) with chat_id={test_chat_id} ---"
    )
    ...
    # Wait for the final follow-up task to complete (it should be cancelled)
    await wait_for_tasks_to_complete(
        engine=db_engine,
...
async def test_schedule_recurring_callback(
    ...
    test_chat_id = get_unique_test_chat_id()
    logger.info(
        f"\n--- Running Recurring Callback Test ({test_run_id}) with chat_id={test_chat_id} ---"
    )
...
@pytest.mark.asyncio
async def test_list_pending_actions(db_engine: AsyncEngine) -> None:
    """
    Tests the list_pending_actions tool:
    1. Schedule multiple callbacks
    2. Mock LLM calls list_pending_actions
    ...
    test_chat_id = get_unique_test_chat_id()
    logger.info(
        f"\n--- Running List Pending Callbacks Test ({test_run_id}) with chat_id={test_chat_id} ---"
    )
    ...
    # --- Part 2: Test list_pending_actions ---
    logger.info("--- Part 2: Testing list_pending_actions ---")
    async with DatabaseContext(engine=db_engine) as db_context:
        result = await processing_service.handle_chat_interaction(
            db_context=db_context,
            chat_interface=mock_chat_interface,
    ...
    logger.info(
        "Verified list_pending_actions returns only conversation-specific callbacks"
    )
