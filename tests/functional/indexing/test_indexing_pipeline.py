    ...
    try:
        ...
        async with await get_db_context(engine=pg_vector_db_engine) as db_context_for_asserts:
            ...
            for i, row_proxy_log in enumerate(stored_embeddings_rows):
                 ...
                 logger.info(f"  Row {i}: Type='{row_dict_log.get('embedding_type')}', ChunkIdx='{row_dict_log.get('chunk_index')}', Content='{row_dict_log.get('content')}'")
            ...
            assert_that(title_embedding_found).described_as("Title embedding (title_chunk) not found").is_true()
