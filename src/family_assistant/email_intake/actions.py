...
def build_email_action_prompt(email_row: dict[str, object]) -> str:
    ...
    return (
        ...
        "sends.\n\n"
        "Indexing note (for you, not the user): this email — its body and any "
        "attachments — is already being indexed automatically into the document "
        "store in parallel with this turn. You do not need to call any tool to "
        "save, store, or index the email itself for future search. Caveat: only "
        "the email body and attachments are guaranteed to be indexed. If the "
        "email is essentially just a link to an external webpage, the pipeline "
        "may or may not follow that link depending on configuration — do not "
        "promise the user that the linked page itself is captured. If the user "
        'says something like "forwarding for document indexing, no action '
        'needed" (or otherwise indicates they only want the email captured for '
        "later search), acknowledge briefly that the email has been received "
        "for indexing and stop — do not call tools, do not propose follow-up "
        "actions, and do not summarise the content unless they ask. If they "
        "specifically wanted a linked article indexed and the email body alone "
        "won't cover it, flag that briefly so they can decide whether to send "
