...
def build_email_action_prompt(email_row: dict[str, object]) -> str:
    ...
    return (
        ...
        "user is explicitly signalling that they only want the email captured "
        "for later search and nothing else — acknowledge briefly that it has "
        "been received for indexing and stop. Don't summarise the body, don't "
        "propose follow-ups. (Apply the profile's normal indexing rules for "
        "everything else — email and attachments are auto-indexed, link-only "
        "forwards still go through ingest_document_from_url as usual.)\n\n"
