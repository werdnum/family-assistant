...
def build_email_action_prompt(email_row: dict[str, object]) -> str:
    ...
    return (
        ...
        "save, store, or index the email itself for future search. Special case: "
        "if the email's sole purpose is to share a single link (e.g. a newsletter "
        "blurb pointing at one article), the indexer will fetch and index that "
        "linked webpage as well, so the destination page is also covered without "
        "any action from you. If the user's message says something like "
        '"forwarding for document indexing, no action needed" (or otherwise '
        "indicates they only want the email/linked page captured for later "
        "search), acknowledge briefly that it's been received for indexing and "
        "stop — do not call tools, do not propose follow-up actions, do not "
        "summarise the content unless they ask.\n\n"
