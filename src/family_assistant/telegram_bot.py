            ...
            async with db_context_getter as db_context:
                ...
                async with self._typing_notifications(context, chat_id):
                    ...
                    for msg_dict in generated_turn_messages:
                        ...
                        # Ensure required fields for add_message_to_history are present
                        if "timestamp" not in msg_to_save:
                            msg_to_save["timestamp"] = datetime.now(timezone.utc)
                        if "interface_message_id" not in msg_to_save:
                            # For messages generated by the system (assistant, tool) that don't directly
                            # correspond to a new interface message ID from the platform.
                            msg_to_save["interface_message_id"] = None
