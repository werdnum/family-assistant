...
class ChatInteractionResult:
    """Result from processing a chat interaction."""
...
class ProcessingService:
    ...
    async def handle_chat_interaction(
        ...
        try:
            ...
            # --- 3. Call Core LLM Processing (streaming to collect attachments) ---
            generated_turn_messages = []
            final_reasoning_info = None
            ...
            async for event, message_dict in self.process_message_stream(
                ...
                # Collect messages that should be saved
                if message_dict and message_dict.get("role"):
                    generated_turn_messages.append(message_dict)

                # Extract reasoning info from done events
                if (
                    event.type == "done"
                    and event.metadata
                    and "message" in event.metadata
                ):
                    assistant_msg = event.metadata["message"]
                    if assistant_msg.get("reasoning_info"):
                        final_reasoning_info = assistant_msg["reasoning_info"]

                # Collect attachment metadata from tool results
                if (
                    event.type == "tool_result"
                    and event.metadata
                    and "attachments" in event.metadata
                ):
                    tool_attachments = event.metadata["attachments"]
                    if isinstance(tool_attachments, list):
                        collected_attachments.extend(tool_attachments)
                    logger.info(
                        f"Collected {len(tool_attachments)} attachments from tool result"
                    )
            ...
            return ChatInteractionResult(
                final_text=final_text_reply,
                final_assistant_message_id=final_assistant_message_internal_id,
                reasoning_info=final_reasoning_info,
                error_traceback=None,
            ...
            return ChatInteractionResult(
                ...
                final_assistant_message_id=error_message_internal_id,
                reasoning_info=None,
                error_traceback=processing_error_traceback,
            )
