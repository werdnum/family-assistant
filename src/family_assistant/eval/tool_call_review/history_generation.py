...
class BatchExecutionError(RuntimeError):
    """A bounded batch attempt could not produce a valid model response."""
...
class BatchProcessError(BatchExecutionError):
    """Pi or its bounded event stream failed before response validation."""


def _is_process_failure(exc: BatchExecutionError) -> bool:
    return isinstance(exc, BatchProcessError) or exc.code in _PROCESS_FAILURE_CODES
...
def _is_delegation_shape(
    ...
    if descriptor_registry is None:
        return False
    return any(
        descriptor_registry.get(tool_name) is not None
        and ToolTag.DELEGATION in descriptor_registry[tool_name].tags
...
class BatchRunner:
    ...
    async def run(self, prompt: str) -> str:
        ...
        try:
            ...
            raise BatchProcessError("process_error") from exc
        assert process is not None
        if returncode != 0:
            raise BatchProcessError("process_exit")
        events: list[object] = []
        for line in stdout.splitlines():
            try:
                events.append(json.loads(line))
            except json.JSONDecodeError:
                raise BatchProcessError("malformed_event") from None
        message_events = [
            event
            for event in events
        ...
        if not message_events:
            raise BatchProcessError("missing_terminal_message")
        ...
        if isinstance(content, list):
            ...
            if text:
                return text
        raise BatchProcessError("empty_terminal_message")
...
async def _communicate_with_pi_limit(
    ...
    if process.stdin is None or process.stdout is None or process.stderr is None:
        ...
        raise BatchProcessError("process_pipes_missing")
    stdin = process.stdin
    ...
    try:
        ...
        raise BatchProcessError("process_timeout") from exc
    except asyncio.CancelledError:
        await _reap_pi(process, tasks)
        raise
        ...
        raise
    if exceeded:
        raise BatchProcessError("stdout_limit")
