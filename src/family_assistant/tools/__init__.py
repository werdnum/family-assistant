...
_LOCAL_TOOL_IMPLEMENTATIONS: dict[str, ToolImplementation] = {
    ...
    "run_authenticated_site_task": run_authenticated_site_task_tool,
...
LOCAL_TOOL_METADATA_BY_NAME: dict[str, LocalToolMetadata] = {
    ...
    # Deliberately NOT tagged BROWSER: this is the tool that *acquires* an
    # authenticated session, and the admissible set for a profile running
    # inside one is derived from that tag. Tagging it would make an
    # authenticated browser profile able to open a second authenticated
    # session from inside the first.
    "run_authenticated_site_task": _metadata(
        ToolTag.STATE_CHANGING,
        ToolTag.SENSITIVE_DATA,
        ToolTag.EXTERNAL_COMM,
        ToolTag.OUTPUT_UNTRUSTED,
