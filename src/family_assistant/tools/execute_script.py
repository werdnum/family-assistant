...
SCRIPT_TOOLS_DEFINITION: list[dict[str, Any]] = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "• attachment_list(source_type=None, limit=20) - List attachments in conversation\n"
                "• attachment_send(attachment_id, message=None) - Send attachment to user\n\n"
                "**Example Scripts:**\n\n"
                ...
                "```\n\n"
                "4. Working with user attachments:\n"
                "```starlark\n"
                "# Get attachments sent by user\n"
                "user_attachments = attachment_list(source_type='user')\n"
                "if len(user_attachments) > 0:\n"
                "    # Get details of the first attachment\n"
                "    attachment_id = user_attachments[0]['attachment_id']\n"
                "    attachment_info = attachment_get(attachment_id)\n"
                "    \n"
                "    # Process the attachment with a tool\n"
                "    if 'image' in attachment_info['mime_type']:\n"
                "        # Pass attachment ID to image analysis tool\n"
                "        analysis = tools_execute('analyze_image', image_data=attachment_id)\n"
                "        \n"
                "        # Send the original attachment back with analysis\n"
                "        attachment_send(attachment_id, 'Analysis: ' + analysis)\n"
                "    else:\n"
                "        attachment_send(attachment_id, 'Received your attachment!')\n"
                "else:\n"
                "    print('No user attachments found')\n"
                "```\n\n"
