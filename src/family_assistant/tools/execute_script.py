...
SCRIPT_TOOLS_DEFINITION: list[ToolDefinition] = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "**LLM API Functions (one-shot calls for summarisation, data extraction, etc.):**\n"
                "• llm(prompt, system=None, model=None) - Make a one-shot LLM call, returns text\n"
                "• llm_json(prompt, schema=None, system=None, model=None) - LLM call returning parsed JSON\n"
                "  Default model: gemini-3-flash-preview. Override with model parameter.\n\n"
                "```starlark\n"
                "# Simple summarisation\n"
                "summary = llm('Summarise this: ' + long_text)\n\n"
                "# Data extraction with JSON schema\n"
                "data = llm_json(\n"
                "    'Extract contacts from: ' + text,\n"
                "    schema={'type': 'object', 'properties': {'contacts': {'type': 'array', 'items': {'type': 'object', 'properties': {'name': {'type': 'string'}, 'email': {'type': 'string'}}}}}}\n"
                ")\n"
                "```\n\n"
