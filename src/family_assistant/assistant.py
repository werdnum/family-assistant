...
class Assistant:
    ...
    async def setup_dependencies(self) -> None:
        ...
        if selected_model.startswith("gemini/"):
            ...
            if self.config.get("openrouter_api_key"):  # Redundant check, but for clarity
                os.environ["OPENROUTER_API_KEY"] = self.config["openrouter_api_key"]
            logger.info("OpenRouter model selected. OPENROUTER_API_KEY set for LiteLLM.")
        else:
            logger.warning(f"No specific API key validation for model: {selected_model}.")
        ...
        if embedding_model_name == "mock-deterministic-embedder":
            ...
            "all-MiniLM-L6-v2", "other-local-model-name",
        ]:
            try:
                ...
                self.embedding_generator = embeddings.SentenceTransformerEmbeddingGenerator(
                    model_name_or_path=embedding_model_name
                )
            except Exception as e:
                logger.critical(f"Failed to initialize local embedding model '{embedding_model_name}': {e}")
        ...
        logger.info(f"Using embedding generator: {type(self.embedding_generator).__name__} with model: {self.embedding_generator.model_name}")
        ...
        default_service_profile_id = self.config.get("default_service_profile_id", "default_assistant")
        ...
        for profile_conf in resolved_profiles:
            ...
            profile_llm_model = profile_proc_conf_dict.get("llm_model", self.config["model"])
            ...
            enabled_local_tool_names = set(local_tool_implementations.keys()) if local_tools_list_from_config is None else set(local_tools_list_from_config)
            ...
            profile_local_implementations = {name: func for name, func in local_tool_implementations.items() if name in enabled_local_tool_names}
            ...
            enabled_mcp_server_ids = set(all_mcp_servers_config.keys()) if mcp_server_ids_from_config is None and all_mcp_servers_config else set(mcp_server_ids_from_config or [])
            
            profile_mcp_servers_config = {sid: sconf for sid, sconf in all_mcp_servers_config.items() if sid in enabled_mcp_server_ids}
            mcp_timeout_seconds = profile_tools_conf_dict.get("mcp_initialization_timeout_seconds", 60)
            ...
            composite_provider_for_profile = CompositeToolsProvider(providers=[local_provider_for_profile, mcp_provider_for_profile])
            try:
                ...
                raise SystemExit(f"Tool provider init failed for '{profile_id}': {provider_err}") from provider_err
            ...
            notes_provider = NotesContextProvider(get_db_context_func=async_get_db_context_for_provider, prompts=profile_proc_conf_dict["prompts"])
            calendar_provider = CalendarContextProvider(calendar_config=profile_proc_conf_dict["calendar_config"], timezone_str=profile_proc_conf_dict["timezone"], prompts=profile_proc_conf_dict["prompts"])
            known_users_provider = KnownUsersContextProvider(chat_id_to_name_map=profile_chat_id_map, prompts=profile_proc_conf_dict["prompts"])
            ...
            if willyweather_api_key and willyweather_location_id and self.shared_httpx_client:
                weather_provider = WeatherContextProvider(
                    location_id=willyweather_location_id, api_key=willyweather_api_key,
                    prompts=profile_proc_conf_dict["prompts"], timezone_str=profile_proc_conf_dict["timezone"],
                    httpx_client=self.shared_httpx_client,
                )
                context_providers.append(weather_provider)
            
            service_config = ProcessingServiceConfig(
                prompts=profile_proc_conf_dict["prompts"], calendar_config=profile_proc_conf_dict["calendar_config"],
                timezone_str=profile_proc_conf_dict["timezone"], max_history_messages=profile_proc_conf_dict["max_history_messages"],
                history_max_age_hours=profile_proc_conf_dict["history_max_age_hours"], tools_config=profile_tools_conf_dict,
                delegation_security_level=profile_proc_conf_dict.get("delegation_security_level", "confirm"),
                id=profile_id,
            )
            
            processing_service_instance = ProcessingService(
                llm_client=llm_client_for_profile, tools_provider=confirming_provider_for_profile,
                service_config=service_config, context_providers=context_providers,
                server_url=self.config["server_url"], app_config=self.config,
            )
        ...
        for service_instance in self.processing_services_registry.values():
            service_instance.set_processing_services_registry(self.processing_services_registry)
        
        fastapi_app.state.processing_services = self.processing_services_registry
        
        self.default_processing_service = self.processing_services_registry.get(default_service_profile_id)
        if not self.default_processing_service:
            logger.warning(f"Default service profile ID '{default_service_profile_id}' not found. Falling back to first available.")
            default_service_profile_id = next(iter(self.processing_services_registry.keys()))
            self.default_processing_service = self.processing_services_registry[default_service_profile_id]
        ...
        fastapi_app.state.tools_provider = self.default_processing_service.tools_provider
        fastapi_app.state.tool_definitions = await self.default_processing_service.tools_provider.get_tool_definitions()
        logger.info(f"Default processing service set to profile ID: '{default_service_profile_id}'.")
        ...
        self.document_indexer = DocumentIndexer(
            pipeline_config=pipeline_config, llm_client=self.default_processing_service.llm_client,
            embedding_generator=self.embedding_generator, scraper=self.scraper_instance,
        )
    ...
    async def start_services(self) -> None:
        ...
        default_profile_conf = next(p for p in self.config["service_profiles"] if p["id"] == self.default_processing_service.service_config.id)

        self.task_worker_instance = TaskWorker(
            processing_service=self.default_processing_service,
            chat_interface=self.telegram_service.chat_interface,
            new_task_event=self.new_task_event,
            calendar_config=default_profile_conf["processing_config"]["calendar_config"],
        ...
        self.task_worker_instance.register_task_handler("log_message", task_wrapper_handle_log_message)
        if self.document_indexer:
            self.task_worker_instance.register_task_handler("process_uploaded_document", self.document_indexer.process_document)
        if self.email_indexer:
            self.task_worker_instance.register_task_handler("index_email", self.email_indexer.handle_index_email)
        self.task_worker_instance.register_task_handler("llm_callback", handle_llm_callback)
        self.task_worker_instance.register_task_handler("embed_and_store_batch", handle_embed_and_store_batch)
        logger.info(f"Registered task handlers for worker {self.task_worker_instance.worker_id}")
        asyncio.create_task(self.task_worker_instance.run())  # Removed new_task_event from run()
    ...
    def initiate_shutdown(self, signal_name: str) -> None:
        ...
        if not self.shutdown_event.is_set():
            logger.warning(f"Received signal {signal_name}. Initiating shutdown via Assistant...")
            self.shutdown_event.set()
        else:
            logger.warning(f"Shutdown already in progress. Signal {signal_name} received again.")
    ...
    async def stop_services(self) -> None:
        ...
        elif self.default_processing_service and self.default_processing_service.tools_provider:
             logger.warning("Processing services registry not found, closing default tools_provider.")
