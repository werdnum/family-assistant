...
async def _format_system_prompt(
    ...
    *,
    ...
    voice_instruction = (
        ...
        "slower than it was.\n"
        "You cannot tell how much time has passed, so do not try to judge when another "
        f"update is due. When one is, a `{VOICE_SILENCE_REMINDER_KEY}` field appears "
        "alongside a tool result to tell you so; only then say something further, and "
        "keep it to a few words. Never read that field aloud or mention that you were "
        "prompted.\n"
