...
class TextChunker(ContentProcessor):
    ...
    def __init__(
        ...
        if chunk_overlap >= chunk_size and chunk_size > 0 : # Ensure overlap is less than chunk size
            logger.warning(f"Chunk overlap ({chunk_overlap}) is >= chunk size ({chunk_size}). Setting overlap to {chunk_size // 2}.")
    ...
    def _split_recursively(self, text: str, separators: Sequence[str]) -> List[str]:
        ...
        for part in splits:
            if not part: # Skip empty parts
                continue
            if len(part) <= self.chunk_size / 2 and not remaining_separators : # Heuristic: if small enough and no more specific separators, keep.
    ...
    def _chunk_text_natively(self, text: str) -> List[str]:
        ...
        while current_pos < text_len:
            ...
            if current_pos >= end_pos : # If no progress or went backward due to large overlap
