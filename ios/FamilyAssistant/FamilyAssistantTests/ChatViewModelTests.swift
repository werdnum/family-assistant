...
final class ChatViewModelTests: XCTestCase {
    ...
    func testSendDraftDoesNotOverwriteUnreconciledSuspendedSession() async throws {
        // Guard the mutation path, not just canSendDraft: a send attempt (including a
        // queued follow-up steer draining through sendDraft) while a suspended
        // session awaits reconciliation must NOT overwrite the preserved session and
        // start an overlapping durable turn.
        let stream = HangingStream()
        let postedTurnID = AtomicString()
        ...
        ChatMockBackendURLProtocol.respond { request in
            ...
            case ("GET", "/api/v1/chat/conversations/web_conv_guard/stream"):
                return .hangingStream(
                    """
                    event: turn_started
                    data: {"turn_id":"\(postedTurnID.value ?? "turn-guard")","seq":0}
        ...
        try await waitUntil { model.isStreaming && postedTurnID.value != nil }
        let sessionBefore = try XCTUnwrap(model.activeTurnSessionForTesting)
        let postsAfterFirstSend = postCount.value

        model.suspendActiveSend()
        XCTAssertFalse(model.isStreaming)

        // Attempt a second send while the suspended session is unreconciled.
        model.draftText = "Second"
        await model.sendDraft()

        // The preserved session is untouched and no overlapping turn was posted.
        let sessionAfter = try XCTUnwrap(model.activeTurnSessionForTesting)
        XCTAssertTrue(
            sessionBefore === sessionAfter,
            "sendDraft must not overwrite a suspended session awaiting reconciliation."
        )
        XCTAssertEqual(
            postCount.value,
            postsAfterFirstSend,
            "No overlapping turn may be posted during the suspend-reconcile window."
        )
    }
