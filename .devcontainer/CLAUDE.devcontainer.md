...
1. **postgres** - PostgreSQL database with pgvector extension
   ...
   - Data stored in the `postgres-data` named volume, which survives container restarts
...
- **postgres-data**: `/var/lib/postgresql/data` (named volume) - Database files, kept across
  restarts
- **dev-logs**: `/var/log/family-assistant` (named volume) - Shared log directory, kept across
  restarts
