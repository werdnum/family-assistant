...
def upgrade() -> None:
    """Upgrade schema."""
    # Existing vector migration - keep as is
    # ### commands auto generated by Alembic - please adjust! ###
    bind = op.get_bind()
    if bind.dialect.name != 'sqlite':
        op.alter_column('document_embeddings', 'embedding',
                   existing_type=sa.NUMERIC(),
                   type_=pgvector.sqlalchemy.vector.VECTOR(dimensions=1536), # Be explicit about dimensions if possible
                   existing_nullable=False)
        # Index creation seems fine without batch
        op.create_index('idx_doc_embeddings_document_chunk', 'document_embeddings', ['document_id', 'chunk_index'], unique=False, if_not_exists=True)
...
def downgrade() -> None:
    ...
    # Revert vector migration changes
    # ### commands auto generated by Alembic - please adjust! ###
    bind = op.get_bind()
    if bind.dialect.name != 'sqlite':
        op.drop_index('idx_doc_embeddings_document_chunk', table_name='document_embeddings', if_exists=True)
        op.alter_column('document_embeddings', 'embedding',
