...
from sqlalchemy import (
    Table, Column, String, Integer, Text, DateTime, JSON, select, insert, update, desc
)
    ...
    for attempt in range(max_db_retries):
        try:
            async with engine.connect() as conn:
                ...
                await conn.commit()
                logger.info(f"Enqueued task {task_id} (Type: {task_type}, Original: {values_to_insert['original_task_id']}, Recurrence: {'Yes' if recurrence_rule else 'No'}).")
            ...
            logger.warning(f"DBAPIError in enqueue_task (attempt {attempt + 1}/{max_db_retries}): {e}. Retrying...")
            ...
            async with engine.connect() as conn:
                async with conn.begin():
                    stmt = (
                        ...
                        .where((tasks_table.c.scheduled_at == None) | (tasks_table.c.scheduled_at <= now)) # noqa: E711
                        .where(tasks_table.c.retry_count < tasks_table.c.max_retries)
                        .order_by(tasks_table.c.retry_count.asc(), tasks_table.c.created_at.asc())
                        .limit(1)
                    ...
                    if task_row:
                        ...
                        if update_result.rowcount == 1:
                            ...
                            logger.warning(f"Worker {worker_id} failed to lock task {task_row.task_id} after selection.")
                            return None # Transaction rollback handles cleanup
                    else:
                        return None # No suitable task found
        except DBAPIError as e:
            logger.warning(f"DBAPIError in dequeue_task (attempt {attempt + 1}/{max_db_retries}): {e}. Retrying...")
            if attempt == max_db_retries - 1:
...
async def update_task_status(task_id: str, status: str, error: Optional[str] = None) -> bool:
    ...
    for attempt in range(max_db_retries):
        try:
            async with engine.connect() as conn:
                stmt = (update(tasks_table).where(tasks_table.c.task_id == task_id).values(**values_to_update))
                result = await conn.execute(stmt)
                await conn.commit()
                if result.rowcount > 0:
                    logger.info(f"Updated task {task_id} status to {status}.")
                    return True
                logger.warning(f"Task {task_id} not found or status unchanged when updating to {status}.")
                return False
        except DBAPIError as e:
            logger.warning(f"DBAPIError in update_task_status (attempt {attempt + 1}/{max_db_retries}): {e}. Retrying...")
            if attempt == max_db_retries - 1:
    ...
    for attempt in range(max_db_retries):
        try:
            async with engine.connect() as conn:
                ...
                await conn.commit()
                if result.rowcount > 0:
                    logger.info(f"Rescheduled task {task_id} for retry {new_retry_count} at {next_scheduled_at}.")
                    return True
                logger.warning(f"Task {task_id} not found when rescheduling for retry.")
                return False
        except DBAPIError as e:
            logger.warning(f"DBAPIError in reschedule_task_for_retry (attempt {attempt + 1}/{max_db_retries}): {e}. Retrying...")
            if attempt == max_db_retries - 1:
                logger.error(f"Max retries exceeded for reschedule_task_for_retry({task_id}). Raising error.")
                raise
            delay = base_delay * (2**attempt) + random.uniform(0, base_delay)
            await asyncio.sleep(delay)
        except ValueError:
            ...
            logger.error(f"Non-retryable error in reschedule_task_for_retry({task_id}): {e}", exc_info=True)
            raise
    raise RuntimeError(f"Database operation failed for reschedule_task_for_retry({task_id}) after multiple retries")
...
async def get_all_tasks(limit: int = 100) -> List[Dict[str, Any]]:
    ...
    for attempt in range(max_db_retries):
        try:
            ...
            logger.warning(f"DBAPIError in get_all_tasks (attempt {attempt + 1}/{max_db_retries}): {e}. Retrying...")
            if attempt == max_db_retries - 1:
            ...
            raise
    raise RuntimeError("Database operation failed for get_all_tasks after multiple retries")
