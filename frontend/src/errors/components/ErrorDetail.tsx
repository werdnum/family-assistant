...
const ErrorDetail: React.FC = () => {
  ...
  const [error, setError] = useState<ErrorDetailData | null>(null);
  ...
  useEffect(() => {
    const fetchErrorDetail = async () => {
      ...
      try {
        ...
        const data: ErrorDetailData = await response.json();
        setError(data);
      } catch (err: any) {
  ...
  const getLevelBadgeClass = (level: ErrorDetailData['level']): string => {
    switch (level) {
      case 'CRITICAL':
