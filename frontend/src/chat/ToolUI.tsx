...
import { Attachment, getAttachmentKey } from '../types/attachments';
...
interface ToolUIProps extends BaseToolUIProps {
  toolName: string;
}
...
const ToolAttachmentDisplay: React.FC<{ attachment: Attachment }> = ({ attachment }) => {
...
const ToolFallback: React.FC<ToolUIProps> = ({ toolName, args, result, status, attachments }) => {
...
export const AddOrUpdateNoteToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  return (
    <div className="tool-call-container tool-note" data-ui="tool-call-content">
      ...
      <div className="tool-note-content">
        {args?.title && <h4 className="tool-note-title">{args.title as string}</h4>}
        {args?.content && <p className="tool-note-text">{args.content as string}</p>}
      </div>
...
export const SearchDocumentsToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  return (
    <div className="tool-call-container tool-search" data-ui="tool-call-content">
      ...
      {args?.query && (
        <div className="tool-search-query">
          Searching for: <strong>{args.query as string}</strong>
        </div>
      )}
...
export const GetNoteToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
      ...
      <div className="tool-get-note-content">
        {args?.title && (
          <div className="tool-get-note-search">
            Looking for: <strong>"{args.title as string}"</strong>
          </div>
        )}
        ...
        {result && parseError && (
          <div className="tool-get-note-raw-result">
            <div className="tool-section-label">Result:</div>
            <div className="tool-result-text">{result as string}</div>
          </div>
        )}
      </div>
...
export const ListNotesToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result array if it's a string
  let parsedResult: Note[] | null = null;
  let parseError = false;
...
export const DeleteNoteToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result JSON if it's a string
  let parsedResult: { success: boolean; message: string } | null = null;
  let parseError = false;
      ...
      <div className="tool-delete-note-content">
        {args?.title && (
          <div className="tool-delete-note-target">
            Deleting: <strong>"{args.title as string}"</strong>
          </div>
        )}
...
export const ScheduleReminderToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
      ...
      <div className="tool-reminder-content">
        {args?.message && (
          <div className="tool-reminder-message">
            <strong>{args.message as string}</strong>
          </div>
        )}

        {args?.time && <div className="tool-reminder-time">📅 {formatTime(args.time as string)}</div>}
...
export const ScheduleFutureCallbackToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
  ...
  const formatCallbackData = (callbackData: Record<string, unknown>) => {
      ...
      <div className="tool-callback-content">
        {args?.description && (
          <div className="tool-callback-description">
            <strong>{args.description as string}</strong>
          </div>
        )}

        {args?.callback_time && (
          <div className="tool-callback-time">
            🕐 {formatTime(args.callback_time as string)}
          </div>
        )}

        {args?.callback_data &&
          Object.keys(args.callback_data as Record<string, unknown>).length > 0 && (
            <div className="tool-callback-data">
              <div className="tool-section-label">Callback Data:</div>
              <div className="tool-callback-data-list">
                {formatCallbackData(args.callback_data as Record<string, unknown>)}
              </div>
            </div>
          )}
      </div>
...
export const ScheduleRecurringTaskToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
  // Helper function to display task parameters in a readable format
  const formatTaskParameters = (taskParams: Record<string, unknown>) => {
  ...
  const formatSchedulePattern = (pattern: string) => {
    if (!pattern) {
      return 'Unknown schedule';
    }

    // Common patterns to make more readable
    const readablePatterns: Record<string, string> = {
      ...
      <div className="tool-recurring-task-content">
        {args?.task_name && (
          <div className="tool-task-name">
            <strong>{args.task_name as string}</strong>
          </div>
        )}

        {args?.schedule_pattern && (
          <div className="tool-task-schedule">
            📅 {formatSchedulePattern(args.schedule_pattern as string)}
          </div>
        )}

        {args?.description && (
          <div className="tool-task-description">{args.description as string}</div>
        )}

        {args?.task_parameters &&
          Object.keys(args.task_parameters as Record<string, unknown>).length > 0 && (
            <div className="tool-task-parameters">
              <div className="tool-section-label">Parameters:</div>
              <div className="tool-task-parameters-list">
                {formatTaskParameters(args.task_parameters as Record<string, unknown>)}
              </div>
            </div>
          )}
...
export const ListPendingCallbacksToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result array if it's a string
  let parsedResult: Callback[] | null = null;
  let parseError = false;
  ...
  const getCallbackStatusIcon = (callback: Callback) => {
  ...
  return (
    ...
    >
      ...
      <div className="tool-list-callbacks-params">
        {args?.limit && (
          <div className="tool-filter-indicator">📊 Limit: {args.limit as number} callbacks</div>
        )}
      </div>
              ...
              {callbacks.map((callback, index) => (
                <div key={index} className="tool-callback-item">
                  ...
                  {callback.callback_data &&
                    Object.keys(callback.callback_data).length > 0 && (
...
export const ModifyPendingCallbackToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
  ...
  const isSuccess =
    ...
    (!result || !(result as string).toString().toLowerCase().includes('error'));
  ...
  return (
    ...
    >
      ...
      <div className="tool-modify-callback-content">
        {args?.callback_id && (
          <div className="tool-callback-id">
            Callback ID: <strong>{args.callback_id as string}</strong>
          </div>
        )}

        {args?.changes && Object.keys(args.changes as Record<string, unknown>).length > 0 && (
          <div className="tool-modify-changes">
            <div className="tool-section-label">Changes:</div>
            <div className="tool-modify-changes-list">
              {formatChanges(args.changes as Record<string, unknown>)}
            </div>
          </div>
        )}
...
export const CancelPendingCallbackToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
  ...
  return (
    ...
    >
      ...
      <div className="tool-cancel-callback-content">
        {args?.callback_id && (
          <div className="tool-callback-id">
            Cancelling Callback ID: <strong>{args.callback_id as string}</strong>
          </div>
        )}
        ...
        {args?.reason && (
          <div className="tool-cancel-reason">
            ...
            <div className="tool-reason-text">{args.reason as string}</div>
          </div>
        )}
      </div>
...
export const ScheduleActionToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Format the time in a human-readable way
  const formatTime = (isoString: string) => {
    if (!isoString) {
      return 'Unknown time';
  ...
  const formatActionData = (actionData: Record<string, unknown>) => {
      ...
      <div className="tool-action-content">
        {args?.action_name && (
          <div className="tool-action-name">
            <strong>{args.action_name as string}</strong>
          </div>
        )}

        {args?.execution_time && (
          <div className="tool-action-time">
            🕐 {formatTime(args.execution_time as string)}
          </div>
        )}

        {args?.action_data &&
          Object.keys(args.action_data as Record<string, unknown>).length > 0 && (
            <div className="tool-action-data">
              <div className="tool-section-label">Parameters:</div>
              <div className="tool-action-data-list">
                {formatActionData(args.action_data as Record<string, unknown>)}
              </div>
            </div>
          )}
...
export const ScheduleRecurringActionToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
  ...
  const formatRecurrenceRule = (rrule: string) => {
    if (!rrule) {
        ...
        <div className="tool-schedule-details">
          {args?.start_time && (
            <div className="tool-schedule-start-time">
              📅 Start Time: <strong>{formatDateTime(args.start_time as string)}</strong>
            </div>
          )}
          ...
          {args?.recurrence_rule && (
            <div className="tool-schedule-recurrence">
              🔁 Recurrence:{' '}
              <strong>{formatRecurrenceRule(args.recurrence_rule as string)}</strong>
              <div className="tool-schedule-rrule">
                <small>RRULE: {args.recurrence_rule as string}</small>
              </div>
            </div>
          )}
          ...
          {args?.action_type && (
            <div className="tool-schedule-action-type">
              ⚡ Action Type: <strong>{args.action_type as string}</strong>
            </div>
          )}

          {args?.task_name && (
            <div className="tool-schedule-task-name">
              🏷️ Task Name: <strong>{args.task_name as string}</strong>
            </div>
          )}

          {args?.action_config &&
            Object.keys(args.action_config as Record<string, unknown>).length > 0 && (
              <div className="tool-schedule-action-config">
                ...
                <pre className="tool-code-block">
                  {JSON.stringify(args.action_config, null, 2)}
                </pre>
              </div>
            )}
        </div>
...
export const GetFullDocumentContentToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
  ...
  const shouldTruncate =
    result && typeof result === 'string' && result.length > MAX_PREVIEW_CHARS;
  ...
  const displayContent =
    shouldTruncate && !isExpanded
      ? (result as string).substring(0, MAX_PREVIEW_CHARS) + '...'
      : result;
      ...
      <div className="tool-document-content">
        {args?.document_id && (
          <div className="tool-document-id">
            Document ID: <strong>{args.document_id as string}</strong>
          </div>
        )}
        ...
        {result && (
          <div className="tool-document-result">
            {isError ? (
              <div className="tool-document-error">❌ {result as string}</div>
            ) : (
              <div className="tool-document-success">
                ...
                <div className="tool-document-text">
                  ...
                  <div className="tool-document-content-display">
                    <pre>{displayContent as string}</pre>
                    {shouldTruncate && (
                      <Button
                        variant="ghost"
                        size="sm"
...
export const IngestDocumentFromUrlToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
  ...
  const fileType = args?.url ? getFileTypeFromUrl(args.url as string) : 'Document';
  const domain = args?.url ? getDomainFromUrl(args.url as string) : '';
  const truncatedUrl = args?.url ? truncateUrl(args.url as string) : '';
  ...
  return (
    ...
    >
      ...
      <div className="tool-ingest-url-content">
        {args?.url && (
          <div className="tool-ingest-url-details">
            <div className="tool-ingest-url-main">
              ...
              <div className="tool-ingest-url-address" title={args.url as string}>
                🌐 {truncatedUrl}
              </div>
        ...
        {args?.metadata &&
          Object.keys(args.metadata as Record<string, unknown>).length > 0 && (
            <div className="tool-ingest-metadata">
              <div className="tool-section-label">Metadata:</div>
              <div className="tool-metadata-items">
                {Object.entries(args.metadata as Record<string, unknown>).map(([key, value]) => (
        ...
        {result && (
          <div className="tool-ingest-result">
            {isError ? (
              <div className="tool-ingest-error">❌ {result as string}</div>
            ) : isSuccess ? (
              <div className="tool-ingest-success">✅ {result as string}</div>
            ) : (
              <div className="tool-ingest-info">{result as string}</div>
            )}
          </div>
        )}
      </div>
...
export const GetUserDocumentationContentToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
      ...
      <div className="tool-user-docs-content">
        {args?.filename && (
          <div className="tool-user-docs-request">
            Reading documentation file: <strong>{args.filename as string}</strong>
          </div>
        )}
            ...
            {status?.type === 'incomplete' ? (
              ...
              <div className="tool-user-docs-content-display">
                ...
                <div className="tool-result-text">
                  {(result as string).length > 1000 ? (
                    <>
                      <div>{(result as string).substring(0, 1000)}...</div>
                      <div className="tool-content-truncated">
                        Content truncated (showing first 1000 characters of{' '}
...
export const QueryRecentEventsToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result JSON if it's a string
  let parsedResult: any = null;
  let parseError = false;
  ...
  const truncateEventData = (data: unknown, maxLength = 200) => {
    ...
    >
      ...
      <div className="tool-query-events-params">
        {args?.source_id && (
          <div className="tool-filter-indicator">
            {getSourceIcon(args.source_id as string)} Source:{' '}
            <strong>{args.source_id as string}</strong>
          </div>
        )}
        {args?.hours && (
          <div className="tool-filter-indicator">
            🕐 Hours: <strong>{args.hours as number}</strong>
          </div>
        )}
        {args?.limit && (
          <div className="tool-filter-indicator">
            📊 Limit: <strong>{args.limit as number}</strong> events
          </div>
        )}
      </div>
      ...
      {result && !parseError && (
        <div className="tool-query-events-results">
          {hasError ? (
            <div className="tool-error-message">❌ {result as string}</div>
...
export const TestEventListenerToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result JSON if it's a string
  let parsedResult: any = null;
  let parseError = false;
      ...
      <div className="tool-test-listener-content">
        {args?.source && (
          <div className="tool-listener-source">
            {getSourceIcon(args.source as string)} Testing source:{' '}
            <strong>{args.source as string}</strong>
          </div>
        )}

        {args?.hours && (
          <div className="tool-test-timerange">
            ⏱️ Looking back: <strong>{args.hours as number} hours</strong>
          </div>
        )}

        {args?.match_conditions &&
          Object.keys(args.match_conditions as Record<string, unknown>).length > 0 && (
            <div className="tool-test-conditions">
              <div className="tool-section-label">Match Conditions:</div>
              <pre className="tool-code-block">
                {JSON.stringify(args.match_conditions, null, 2)}
              </pre>
            </div>
          )}
      </div>
                ...
                <div className="tool-test-success">
                  ...
                  {parsedResult.analysis && parsedResult.analysis.length > 0 && (
                    <div className="tool-test-analysis">
                      ...
                      {parsedResult.analysis.map((item: any, index: number) => (
...
export const CreateEventListenerToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result JSON if it's a string
  let parsedResult: any = null;
  ...
  const formatMatchConditions = (conditions: Record<string, unknown>) => {
      ...
      <div className="tool-create-listener-content">
        {args?.name && (
          <div className="tool-listener-name">
            <strong>{args.name as string}</strong>
          </div>
        )}
        ...
        <div className="tool-listener-config">
          {args?.source && (
            <div className="tool-listener-source">
              {getSourceIcon(args.source as string)} Source:{' '}
              <strong>{args.source as string}</strong>
            </div>
          )}
          ...
          {args?.action_type && (
            <div className="tool-listener-action">
              {getActionTypeIcon(args.action_type as string)} Action:{' '}
              <strong>{args.action_type as string}</strong>
            </div>
          )}
        ...
        {args?.condition_script && (
          <div className="tool-listener-condition-script">
            ...
            <pre className="tool-code-block">{args.condition_script as string}</pre>
          </div>
        )}
        ...
        {args?.script_code && (
          <div className="tool-listener-script">
            ...
            <pre className="tool-code-block">
              {(args.script_code as string).substring(0, 200)}
              {(args.script_code as string).length > 200 ? '...' : ''}
            </pre>
          </div>
        )}
...
export const ListEventListenersToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result JSON if it's a string
  let parsedResult: any = null;
  let parseError = false;
  ...
  const getListenerStatusIcon = (enabled: boolean, oneTime: boolean) => {
    if (!enabled) {
      return '⏸️';
    }
    if (oneTime) {
    ...
    >
      ...
      {result && !parseError && (
        <div className="tool-list-listeners-results">
          {isError ? (
            <div className="tool-error-message">❌ {parsedResult?.message || (result as string)}</div>
          ) : listeners.length > 0 ? (
...
export const DeleteEventListenerToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result JSON if it's a string
  let parsedResult: any = null;
  let parseError = false;
      ...
      <div className="tool-delete-listener-content">
        {args?.listener_id && (
          <div className="tool-listener-id-target">
            Deleting Listener ID: <strong>{args.listener_id as string}</strong>
          </div>
        )}
        ...
        {result && !parseError && (
          <div className="tool-delete-listener-result">
            {isError ? (
              <div className="tool-delete-error">
                ❌ {parsedResult?.message || (result as string)}
              </div>
            ) : isSuccess ? (
...
export const ToggleEventListenerToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result JSON if it's a string
  let parsedResult: any = null;
  let parseError = false;
  ...
  const statusDisplay =
    args?.enabled !== undefined
      ? getStatusDisplay(args.enabled as boolean)
    ...
    >
      ...
      <div className="tool-toggle-listener-content">
        <div className="tool-toggle-action">
          {args?.listener_id && (
            <div className="tool-listener-id-target">
              Listener ID: <strong>{args.listener_id as string}</strong>
            </div>
          )}
        ...
        {result && !parseError && (
          <div className="tool-toggle-listener-result">
            {isError ? (
              <div className="tool-toggle-error">
                ❌ {parsedResult?.message || (result as string)}
              </div>
            ) : isSuccess ? (
...
export const ValidateEventListenerScriptToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
      ...
      <div className="tool-validate-script-content">
        {args?.script_code && (
          <div className="tool-script-preview">
            ...
            <pre className="tool-code-block">{args.script_code as string}</pre>
          </div>
        )}
      </div>
...
export const TestEventListenerScriptToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
      ...
      <div className="tool-test-script-content">
        ...
        {args?.timeout && (
          <div className="tool-test-timeout">
            ⏱️ Timeout: <strong>{args.timeout as number} seconds</strong>
          </div>
        )}
      </div>
...
export const RenderHomeAssistantTemplateToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
      ...
      <div className="tool-ha-template-content">
        {args?.template && (
          <div className="tool-ha-template-input">
            ...
            <pre className="tool-code-block">{args.template as string}</pre>
          </div>
        )}
...
export const AddCalendarEventToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Format the time in a human-readable way
  const formatTime = (isoString: string) => {
    if (!isoString) {
      return 'Unknown time';
  ...
  const isMultiDayEvent = (startTime: string, endTime: string, allDay: boolean) => {
      ...
      <div className="tool-calendar-content">
        {args?.summary && (
          <div className="tool-calendar-summary">
            <strong>{args.summary as string}</strong>
          </div>
        )}
        ...
        {args?.start_time && (
          <div className="tool-calendar-time">
            {args?.all_day ? (
              ...
              isMultiDayEvent(args.start_time as string, args.end_time as string, args.all_day as boolean) ? (
                <span>
                  📅 {formatDate(args.start_time as string)} -{' '}
                  {formatDate(args.end_time as string)} (All day)
                </span>
              ) : (
                <span>📅 {formatDate(args.start_time as string)} (All day)</span>
              )
            ) : // Timed event display
            args?.end_time ? (
              <span>
                🕐 {formatTime(args.start_time as string)} -{' '}
                {formatTime(args.end_time as string)}
              </span>
            ) : (
              <span>🕐 {formatTime(args.start_time as string)}</span>
            )}
          </div>
        )}

        {args?.description && (
          <div className="tool-calendar-description">{args.description as string}</div>
        )}
        ...
        {args?.calendar_url && (
          <div className="tool-calendar-location">
            📍 Calendar: {(args.calendar_url as string).split('/').pop() || 'Unknown'}
          </div>
        )}
      </div>
...
export const SearchCalendarEventsToolUI: React.FC<BaseToolUIProps> = ({
  args,
  result,
  status,
}) => {
  // Helper function to parse events from the result string
  const parseEvents = (resultStr: string) => {
  ...
  const getCalendarName = (calendarUrl: string) => {
    if (!calendarUrl) {
  ...
  const events = result ? parseEvents(result as string) : [];
  ...
  return (
    ...
    >
      ...
      <div className="tool-calendar-search-params">
        {args?.search_text && (
          <div className="tool-search-query">
            🔎 <strong>"{args.search_text as string}"</strong>
          </div>
        )}

        {(args?.date_range_start || args?.date_range_end) && (
          <div className="tool-search-daterange">
            📅 {args.date_range_start as string || 'Today'} →{' '}
            {args.date_range_end as string || '3 months'}
          </div>
        )}

        {args?.max_results && (
          <div className="tool-search-limit">📊 Max results: {args.max_results as number}</div>
        )}
      </div>
...
export const ModifyCalendarEventToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Format the time in a human-readable way
  const formatTime = (isoString: string) => {
    if (!isoString) {
      return 'Unknown time';
  ...
  const isSuccess =
    result && typeof result === 'string' && !result.toLowerCase().includes('error');
      ...
      <div className="tool-calendar-content">
        {args?.uid && (
          <div className="tool-calendar-uid">
            <strong>Event ID:</strong> {args.uid as string}
          </div>
        )}

        {args?.calendar_url && (
          <div className="tool-calendar-location">
            📍 Calendar: {getCalendarName(args.calendar_url as string)}
          </div>
        )}
        ...
        <div className="tool-calendar-changes">
          {args?.new_summary && (
            <div className="tool-calendar-change">
              <strong>New Title:</strong> {args.new_summary as string}
            </div>
          )}

          {args?.new_start_time && (
            <div className="tool-calendar-change">
              <strong>New Start:</strong> 🕐 {formatTime(args.new_start_time as string)}
            </div>
          )}

          {args?.new_end_time && (
            <div className="tool-calendar-change">
              <strong>New End:</strong> 🕐 {formatTime(args.new_end_time as string)}
            </div>
          )}

          {args?.new_description !== undefined && (
            <div className="tool-calendar-change">
              <strong>New Description:</strong> {(args.new_description as string) || '(removed)'}
            </div>
          )}
          ...
          {args?.recurrence_rule !== undefined && (
            <div className="tool-calendar-change">
              ...
              {(args.recurrence_rule as string)
                ? `🔄 ${args.recurrence_rule as string}`
                : '(removed)'}
            </div>
          )}
...
export const DeleteCalendarEventToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Get calendar name from URL
  const getCalendarName = (calendarUrl: string) => {
    if (!calendarUrl) {
      return 'Unknown calendar';
...
export const GetMessageHistoryToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Parse the result array if it's a string
  ...
  return (
    ...
    >
      ...
      <div className="tool-message-history-params">
        {args?.interface_type && (
          <div className="tool-filter-indicator">
            {getInterfaceIcon(args.interface_type as string)} Interface:{' '}
            <strong>{args.interface_type as string}</strong>
          </div>
        )}
        {args?.user_name && (
          <div className="tool-filter-indicator">
            👤 User: <strong>{args.user_name as string}</strong>
          </div>
        )}
        {args?.limit && (
          <div className="tool-filter-indicator">📊 Limit: {args.limit as number} messages</div>
        )}
      </div>
...
export const SendMessageToUserToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Determine status icon and classes
  let statusIcon = null;
  let statusClass = '';
        ...
        <div className="tool-message-details">
          {args?.target_chat_id && (
            <div className="tool-message-recipient">
              📱 To Chat ID: <strong>{args.target_chat_id as string}</strong>
            </div>
          )}
          ...
          {args?.message_content && (
            <div className="tool-message-content">
              ...
              <div className="tool-message-text">
                {(args.message_content as string).length > 200 ? (
                  <>
                    <div>"{(args.message_content as string).substring(0, 200)}..."</div>
                    <div className="tool-content-truncated">
                      Message truncated (showing first 200 characters of{' '}
                      {(args.message_content as string).length} total)
                    </div>
                  </>
                ) : (
...
export const ExecuteScriptToolUI: React.FC<BaseToolUIProps> = ({ args, result, status }) => {
  // Determine status icon and classes
  let statusIcon = null;
  let statusClass = '';
      ...
      <div className="tool-execute-script-content">
        {args?.script && (
          <div className="tool-script-input">
            ...
            <pre className="tool-code-block">
              {(args.script as string).length > 500 ? (
                <>
                  {(args.script as string).substring(0, 500)}...
                  <div className="tool-content-truncated">
                    Script truncated (showing first 500 characters of{' '}
                    {(args.script as string).length} total)
                  </div>
                </>
              ) : (
                args.script
        ...
        {args?.globals && Object.keys(args.globals as Record<string, unknown>).length > 0 && (
          ...
          <div className="tool-script-result">
            {status?.type === 'incomplete' ? (
              <div className="tool-error-message">
                ...
                <pre className="tool-error-text">{result as string}</pre>
              </div>
            ) : (
              <div className="tool-script-output">
                ...
                <div className="tool-result-text">
                  {(result as string).trim() === '' ? (
                    ...
                    <pre className="tool-script-result-block">{result as string}</pre>
                  )}
                </div>
              </div>
            )}
...
export const toolUIsByName: Record<string, React.FC<any>> = {
