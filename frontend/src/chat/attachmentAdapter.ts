...
type AttachmentType = 'file' | 'image' | 'document';
 ...
 * @returns {Promise<UploadResponse>} Upload response with attachment metadata
 */
const uploadFileToService = async (file: File): Promise<UploadResponse> => {
...
export class FileAttachmentAdapter {
   ...
   * @returns {Promise<CustomAttachment>} Attachment object
   */
  async add({ file }: { file: File }): Promise<CustomAttachment> {
    try {
      // Validate the file
      validateFile(file);
   ...
   * @param {CustomAttachment} attachment - The attachment to process
   * @returns {Promise<CustomAttachment>} Processed attachment with content URL
   */
  async send(attachment: CustomAttachment): Promise<CustomAttachment> {
    try {
      if (!attachment.file) {
        throw new Error('File is missing from attachment');
      }
   ...
   * @param {CustomAttachment} attachment - The attachment to remove
   * @returns {Promise<void>}
   */
  async remove(attachment: CustomAttachment): Promise<void> {
    try {
      ...
      console.error(`Error removing attachment from server: ${(error as Error).message}`);
    }
...
export class CompositeAttachmentAdapter {
  adapters: FileAttachmentAdapter[];
  constructor(adapters: FileAttachmentAdapter[] = []) {
   ...
   * @returns {FileAttachmentAdapter|null} Matching adapter or null
   */
  getAdapterForType(type: string): FileAttachmentAdapter | null {
    return (
   ...
   * @param {CustomAttachment} attachment - The attachment to send
   * @returns {Promise<CustomAttachment>} Processed attachment
   */
  async send(attachment: CustomAttachment): Promise<CustomAttachment> {
