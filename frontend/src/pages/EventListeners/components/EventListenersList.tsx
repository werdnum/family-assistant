...
const EventListenersList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchListeners = async (
      ...
      try {
        ...
        const data: ListenersResponse = await response.json();
  ...
  const handleFiltersSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (filters.source_id) {newParams.set('source_id', filters.source_id);}
    if (filters.action_type) {newParams.set('action_type', filters.action_type);}
    if (filters.conversation_id) {newParams.set('conversation_id', filters.conversation_id);}
    if (filters.enabled) {newParams.set('enabled', filters.enabled);}
  ...
  if (loading) {return <div>Loading event listeners...</div>;}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  setFilters({ ...filters, conversation_id: e.target.value })
                }
