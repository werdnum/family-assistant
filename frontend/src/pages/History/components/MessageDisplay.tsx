...
const getAttachmentKey = (attachment: Attachment, index: number): string => {
  return attachment.id || attachment.content_url || `attachment-${index}`;
};

const MessageDisplay: React.FC<MessageDisplayProps> = ({ message }) => {
  const [expandedToolCalls, setExpandedToolCalls] = useState<Set<number>>(new Set());
  const [expandedTraceback, setExpandedTraceback] = useState<boolean>(false);
  ...
  const getRoleIcon = (role: Message['role']): string => {
    switch (role) {
      case 'user':
        return '👤';
      case 'assistant':
