...
**Quick Start** — to enable tracing with an OTLP-compatible collector (e.g. Jaeger, Grafana Tempo):
...
**When disabled** (`OTEL_ENABLED=false`, the default), no-op tracer and meter providers are
explicitly set. No SDK components are loaded, no connections are attempted, and no errors are
produced. You do not need a collector running.

**When enabled**, ensure an OTLP-compatible collector is reachable at the configured endpoint.
Without a collector, you will see gRPC/HTTP connection errors in the logs. To enable OTel without a
collector (e.g. for local development), set the exporter to `console` or `none`:
...
**Docker Compose with Jaeger** (example):
...
See [docs/design/opentelemetry-observability.md](docs/design/opentelemetry-observability.md) for the
full design document including span hierarchy, semantic conventions, and implementation details.
