...
from mcp.types import Tool, TextContent, ImageContent  # ImageContent is used
...
try:
    ...
    print(
        "Warning: Playwright library not found. HTML Scraping will rely on basic HTTP GET and markitdown conversion.",
        file=sys.stderr,
    )
    print(
        "Install using a PEP 723 runner or 'pip install \"markitdown[html]>=0.1.0\" playwright'",
        file=sys.stderr,
    )
    print(
        "Then run: python -m playwright install --with-deps chromium", file=sys.stderr
    )
...
try:
    ...
    print(
        "Error: markitdown is not installed. Run with a PEP 723 runner (e.g., pipx run).",
        file=sys.stderr,
    )
    sys.exit(1)
# --- End Imports ---
...
mcp_scraper_user_agent = (
    f"{APP_NAME}/{APP_VERSION} (MCPTool/{__version__}; +{WEBSITE_URL})"
)
...
class AsyncScraper:
    ...
    def __init__(self, print_error=None, verify_ssl=True):
        ...
        """
        self.print_error = print_error or (
            lambda msg: print(f"SCRAPER: {msg}", file=sys.stderr)
        )
    ...
    async def _convert_bytes_to_markdown(
        self, content_bytes: bytes, filename: Optional[str]
    ) -> Optional[str]:
    ...
    async def scrape(self, url: str) -> Dict[str, Any]:
        ...
        (
            ...
            final_url,
            encoding,
        ) = await self._scrape_with_httpx(url)
        ...
        if mime_type == HTML_MIME_TYPE:
            ...
            if self.playwright_available:
                self.print_error(
                    f"Attempting JS rendering with Async Playwright for {url}"
                )
                playwright_html_str, _ = await self._scrape_with_playwright(
                    url
                )  # Use original URL for Playwright
                if playwright_html_str:
                    ...
                    )  # Encode to bytes for MarkItDown
            ...
            async with async_playwright() as p:
                try:
                    ...
                    context = await browser.new_context(
                        ignore_https_errors=not self.verify_ssl, user_agent=user_agent
                    )
                    ...
                    try:
                        ...
                        )  # Slightly increased timeout
                    except PlaywrightTimeoutError:
                        self.print_error(
                            f"Warning: Async Playwright timed out waiting for network idle at {url}. Content might be incomplete."
                        )
                    ...
                    if "Executable doesn't exist" in str(e):
                        self.print_error(
                            "Playwright browser not found. Please run: python -m playwright install --with-deps chromium"
                        )
                        self.playwright_available = False
                    return None, None
                except Exception as e:
...
async def check_playwright_async():
    ...
    if not _playwright_installed or not async_playwright:
        print(
            "MCP: Playwright library not installed, skipping browser check.",
            file=sys.stderr,
        )
        return False
    browser = None
    try:
        async with async_playwright() as p:
            browser = await p.chromium.launch()
        print(
            "MCP: Async Playwright Chromium browser check successful.", file=sys.stderr
        )
        return True
    except Exception as e:
        print(
            "MCP Warning: Async Playwright Chromium browser check failed.",
            file=sys.stderr,
        )
        if "Executable doesn't exist" in str(e):
            print(
                "MCP Warning: Run: python -m playwright install --with-deps chromium",
                file=sys.stderr,
            )
            print(
                "MCP Warning: Scraping will fall back to basic HTTP GET.",
                file=sys.stderr,
            )
        else:
...
async def serve(verify_ssl: bool = True) -> None:
    ...
    async def call_tool(
        ...
        try:
            ...
            if result_type == "markdown":
                print(
                    f"MCP: Successfully converted content to Markdown for: {url}",
                    file=sys.stderr,
                )
                return [TextContent(type="text", text=result["content"])]
            elif result_type == "text":
                print(
                    f"MCP: Successfully retrieved text content ({result.get('mime_type')}) for: {url}",
                    file=sys.stderr,
                )
                return [TextContent(type="text", text=result["content"])]
            elif result_type == "image":
                print(
                    f"MCP: Successfully retrieved image content ({result.get('mime_type')}) for: {url}",
                    file=sys.stderr,
                )
                return [
                    ImageContent(
                ...
                print(
                    f"MCP: Scraping/processing error for {url}: {error_message}",
                    file=sys.stderr,
                )
                raise McpError(error_message)
            else:
                unknown_error = (
                    f"Unknown result type '{result_type}' from scraper for {url}."
                )
            ...
            print(
                f"MCP: Error during scraping/conversion for {url}: {e}", file=sys.stderr
            )
    ...
    print(
        "MCP Async Web Scraper Server starting. Checking Playwright...", file=sys.stderr
    )
                ...
                if result_type == "markdown" or result_type == "text":
                    ...
                    if args.output_file:
                        try:
                            ...
                            print(
                                f"STANDALONE: Image content ({result.get('mime_type')}) saved to {args.output_file}",
                                file=sys.stderr,
                            )
                        except IOError as e:
                            print(
                                f"STANDALONE: Error writing image to file {args.output_file}: {e}",
                                file=sys.stderr,
                            )
                        ...
                        sys.stdout.flush()
                        print(
                            f"\nSTANDALONE: Image content ({result.get('mime_type')}) written to stdout.",
                            file=sys.stderr,
                        )
                elif result_type == "error":
                    print(
                        f"STANDALONE: Error: {result.get('message', 'Unknown scraping error.')}",
                        file=sys.stderr,
                    )
                    sys.exit(1)
                else:
                    print(
                        f"STANDALONE: Error: Unknown result type '{result_type}' from scraper.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
