...
- `scripts/review-changes.py`: Integrated with guardian plugin's code review workflow. Supports
  three modes: default (staged changes), `--commit` (last commit), and `--branch [BASE]` (whole
  branch vs `BASE`, default `origin/main`) for use as a pre-PR review step.
