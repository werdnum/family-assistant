...
retrying a submission, and a fresh one for a new one — the id identifies the submission on the echo;
it is not a server-side idempotency key, so a retry whose first attempt was queued is injected
twice.
