...
`execute_shell(command="convert in.png -dither out.bmp", cwd="/tmp/workspaces/inkplate")`. A
command built while the script runs, such as an f-string with a file name in it, is reviewed on its
own each time. To keep such a step covered, write the changing data to a file first and have a fixed
command read it.
