...
- **Email:** If configured by the operator, forward order confirmations, tickets, travel bookings,
  ...
  the assistant notifies your primary Telegram chat if one is configured. Email replies are sent
  only when your forwarding sender address is mapped to your account by the operator.
...
- **Forward Email for Action:** If email intake actions are enabled, you can forward an order
  ...
  Web UI, and Telegram is notified when it is your configured primary communication channel. If the
  email came through a recipient-only alias without a mapped forwarding sender, confirmations can
  still be created, but the assistant will not email a reply.
