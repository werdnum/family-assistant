...
- **Shopping and UCP:** For requests like "find me an X online and send me a checkout link," the
  ...
  that serve UCP under a path prefix (its `/.well-known/ucp` is probed at that subpath, not only at
  the site root) — so these resolve without special configuration. If a site publishes no UCP
  profile the assistant says so plainly and stops, rather than guessing an endpoint and reporting a
  confusing error. The server also publishes its own public UCP platform profile at
...
- **iOS App:** If the native iOS app is installed, it opens Family Assistant after secure sign-in.
  The app is organized into five tabs along the bottom: **Chat**, **Voice**, **Notes**,
  **Documents**, and **More**. Chat, Voice, and Notes are native screens; Documents and the items
  under More open the corresponding pages inside the app. Native Chat shares the same conversation
  history as the browser, streams replies as they are generated, supports stopping or steering a
  running reply, supports profile switching (picking a profile starts a fresh conversation in it,
  and reopening an earlier conversation resumes the profile it was started in so its prior messages
