...
- **Monitor Events and Get Automated Notifications:** \*The assistant can now watch for specific
  events and notify you when they happen: \*"Let me know when Andrew arrives home" \*"Alert me if
  ...
  events from home assistant" \*"Test if person.andrew state changes to 'Home' would have triggered
...
- \*\*Setting Up Event Automations:\*\*You can create event automations either by asking the
  ...
  automation: "Test if entity_id equals 'person.andrew' would match recent events" \*Be specific
  with field names - use the exact names you see in the event data \*You can filter by event type:
  "Test if event_type equals 'state_changed' and entity_id equals 'person.andrew'" \*For complex
