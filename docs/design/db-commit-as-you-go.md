...
- **Handle operations refuse to run under an ambient transaction.** The split stops a
  ...
  transaction sets a ContextVar token; a handle operation that sees the token raises immediately,
  **in production, on both backends** — turning a backend-dependent hang into an error at the
  offending call. The token is deliberately **inherited by child tasks with no identity-based
  exemption**: an *awaited* child (`asyncio.gather(helper(db))` inside the block) is exactly the
  dangerous case — on SQLite the parent waits on the child while the child waits on the parent's
  engine lock (deadlock), on PostgreSQL the child silently commits outside the enclosing rollback —
  so inherited handle use must fail fast too. Genuine fire-and-forget work opts out *explicitly at
  spawn* by clearing the token in the child's context (a `spawn_detached()` helper /
  `asyncio.create_task(coro, context=...)`); the DB log handler is the canonical user, and its
  detached writes then merely queue on the SQLite lock and proceed.
...
| Delegation notification + response delivery (`task_worker.py:2034-2097, 2259-2345`) — **implemented as send-then-record** | **As built**: the send happens *before* the block, which then records the delivery and marks notified atomically. This keeps the guarantee the ordering exists for — a failed send leaves `notified_at` NULL with nothing written — without a transaction ever being open across an interface call, and so needs no pre-resolve step in the interfaces. The originally proposed shape was: explicit block spanning add_message → interface send → mark_notified, preserving the documented rollback-on-send-failure that keeps `notified_at` NULL for retry. **Precondition**: the send path must not touch the database from the sending task — today `EmailChatInterface._resolve_target` (`email_intake/outbound.py:210-217`) and the Telegram attachment-content fetch (`telegram/interface.py:266-285`) open their own contexts mid-send, which the ambient-transaction guard would reject (and Telegram's per-attachment `except` would turn into silently dropped attachments). The cutover hoists these into a pre-resolve step: targets resolved and attachment payloads fetched *before* the block opens, with interfaces receiving prepared data |
...
| App-auth token issuance (`app_auth.py:300-321` `/exchange`, `:355-372` `/refresh`)                                        | API-token insert + linked refresh-token insert (exchange); replacement insert + refresh-token relink (refresh). A failed second write commits a live credential whose secret was never returned — and the exchange has already consumed the authorization code                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `handle_llm_callback` delivery (`task_worker.py:788-906`) — **split, not wrapped**                                        | the handler runs a full LLM turn then sends; a failed send raises and the task retries. Today rollback discards the turn so the retry regenerates from a clean slate; under handle semantics the committed turn would be joined by a second one, repeating tool side effects. Restructure like the delegation family: generation commits (it already does — the assistant row is durable), and a delivery checkpoint makes the retry *resume at delivery*, reusing the persisted reply instead of rerunning the LLM. `_repair_unmatched_tool_calls` does not cover this — the first turn is complete, not orphaned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **No transaction spans an LLM call or tool dispatch.** `DatabaseTransaction.__aenter__` sets a
  ...
  `RuleBasedMockLLMClient._record_call` (`tests/mocks/mock_llm.py:119`) asserts it is unset. This
  turns the core architectural rule into a failing test rather than a review comment, forever. The
  companion wrapper at `ToolExecutor.execute` was **not** landed: `ToolExecutionContext.db_context`
  is typed `Database`, so a transaction cannot reach a tool at all, and a tool that reaches its
  handle from inside one is refused by the production guard — both stronger than a test-only assert.
  The residual gap is a tool that only does network I/O inside a transaction, which neither catches.
...
- **Connection-leak teardown check**: pool checkout counter must be zero at test end. *Landed
  reporting-only.* Several web API tests that run a live server still show one connection
  outstanding after a drain window, untraced to a specific holder; an intermittently failing gate is
  worse than a logged count. The duration watchdog already catches a connection held open inside a
  transaction, which is the hazard this design is about. Follow-up.
