...
- **The originating request, for delegated runs.** A delegated subconversation is unattended in the
  ...
  gets the behaviour by construction rather than by remembering to. It reaches only a profile that
  runs *here*, though: the originating request is local authorization context and does not cross the
  A2A boundary, whether the remote target is pollable (which returns at submit, so nothing local
  assembles its reviewer input at all) or synchronous (where `RemoteA2AService` takes the trigger
  and discards it). See the residual below. The rows are read at trigger-build time rather than
  snapshotted at hand-off, so the propagated request carries the provenance the message actually
  has, and only from a turn a human started — and there only from rows a person could have sent,
  since a history read asks for visible rows and an internal one is machine-generated by
  construction. See the unattended-delegation residual below.
  ...
  the gate that admitted the write — a sighted human confirmation, or the reviewer's `allow` verdict
  on the write, in any layer and any `taint_policy.mode` (the judge's `allow` is the same
  computation whether or not enforcement could have blocked; only escalation verdicts require the
  gate to have been real) — cures the authoring taint for the definition's firings instead of
  propagating it through time, with everything uncured keeping exactly this fail-closed behavior.
...
- **Delegating from an unattended turn propagates no originating request unless the chain carries
  one live.** Only a turn a human *started* contributes rows. Every other kind puts machine-composed
  text in the user role — a subconversation's goal, a completion wake's pinned result data, an event
  payload — and a clean one of those is stamped `trusted_user` like any other untainted row. That is
  exactly why the rule is "not a human's turn" rather than "not trusted": reading such a row would
  launder generated text, including any recipient the model put in it, into the field that claims to
  be the human request and into the destination echo that reads it. An unattended turn therefore
  carries the request forward only through what it inherited on its live trigger, which a run
  claimed later by a worker does not have. So a second-level *asynchronous* delegation, and one
  enqueued from a completion wake, review as they did before propagation existed — stubs, with
  ambiguity taking the deferral path — while synchronous chains and every delegation from an
  interactive turn carry the request to a local profile. Closing it means either persisting the
  request with the run or resolving the parent chain at claim time, and neither is worth the storage
  or the recursion for a rare shape whose fallback is the previous, safe behaviour.
- **No remote (A2A) delegation propagates an originating request**, pollable or synchronous, and by
  design rather than by omission: `RemoteA2AService` states that review-trigger metadata is local
  authorization context and sends only the delegated request and taint metadata. A remote profile
  assembles its own reviewer input, so one judging a tainted delegation sees what it saw before this
  change. Closing it means putting the household's own words into an A2A payload, which is a
  disclosure decision about what leaves this deployment rather than a plumbing gap — it belongs to
  whoever owns that contract, not to this change.
