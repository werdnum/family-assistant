...
- **The originating request, for delegated runs.** A delegated subconversation is unattended in the
  ...
  boundary that runs the delegated turn *here* — synchronous, worker-run, and the completion wake —
  routes through, so a new local boundary gets the behaviour by construction rather than by
  remembering to. A pollable remote target is not one of those: it returns at submit and the turn
  runs on the other service, so nothing local assembles its reviewer input (see the residual below).
  The rows are read at trigger-build time rather than snapshotted at hand-off, so the propagated
  request carries the provenance the message actually has, and only from a turn a human started —
  and there only from rows a person could have sent, since a history read asks for visible rows and
  an internal one is machine-generated by construction. See the unattended-delegation residual
  below.
...
- **Delegating from an unattended turn propagates no originating request unless the chain carries
  ...
  interactive turn carry the request. Closing it means either persisting the request with the run or
  resolving the parent chain at claim time, and neither is worth the storage or the recursion for a
  rare shape whose fallback is the previous, safe behaviour.
- **A pollable remote delegation propagates no originating request.** The A2A submit carries the
  goal and taint metadata; the delegated turn then runs on the remote service, which assembles its
  own reviewer input, so a remote profile judging a tainted delegation sees what it saw before this
