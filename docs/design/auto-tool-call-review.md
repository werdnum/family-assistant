...
- **The originating request, for delegated runs.** A delegated subconversation is unattended in the
  ...
  provenance the message actually has, and only from a turn a human started — and there only from
  rows a person could have sent, since a history read asks for visible rows and an internal one is
  machine-generated by construction. See the unattended-delegation residual below.
