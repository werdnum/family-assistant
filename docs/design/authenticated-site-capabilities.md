...
- **Profile tool policy:** the browser processing profile has browser tools, not unrelated
  ...
  indirectly and would undo everything else on this list.
...
to the site's configured visual profile, and the remaining tool list reviewed against the intended
damage envelope. That is standard processing-profile configuration, not a site-specific runtime or a
...
- browser and visual execution use existing static processing-profile IDs, and startup validation
  ...
  the site's configured visual profile, or ambient household context providers not excluded — so
...
- Add the `authenticated_browser_profile` and `authenticated_browser_visual_profile` variants: no
  ...
  to the configured visual profile (which itself delegates to nothing), remaining tools reviewed
  against the envelope. Authenticated runs delegate to them rather than the shipped profiles, and
  startup validation rejects a site configuration naming a profile that violates these constraints.
