...
- `credential_alias` is routing, not release authority: it tells the tool that a stale or absent jar
  should lead to a jarless session and a login attempt rather than `login_required`, and it **pins**
  the one secret this site's runs may ask for — trusted orchestration passes it to browser-server at
  session creation, `browser_autofill` takes no alias argument, and a site without one has no
  autofill at all. Whether that secret is released, for which page origins, and with what approval
  outcome lives entirely in the Keychute policy row for it (autofill section);
...
carrying), with the same confinement supplied explicitly from the complete configured origin set
(`authenticated_origins` plus `navigation_allowlist`), and the run begins at the login form. If the
login raises a challenge a human can complete in the live session (an MFA code, a captcha), the run
takes the existing `handoff_pending` path — the human finishes it in the parked session and handback
resumes the task; `needs_human` is the exit only for what no one can finish mid-session — a bad
password, an SSO redirect out of scope, a hard bot block.
...
2. **family-assistant:** `credential_alias` on `authenticated_sites`, pinned onto the session at
   ...
   to the jarless session; every authenticated-site run, jarless included, excluded from the
   backend's transparent re-provisioning; docs.
