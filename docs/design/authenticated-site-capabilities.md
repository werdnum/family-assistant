...
- `credential_alias` is a routing hint and a default, not an authorization: it tells the tool that a
  stale or absent jar should lead to a jarless session and a login attempt rather than
  `login_required`, and it names the secret the browser profile will ask for by default. Which
  secret may be released, for which page origins, and with what approval outcome lives entirely in
  the Keychute policy row for that secret (autofill section);
...
For a site **with** `credential_alias` (the autofill section), a stale or invalidated jar does not
...
`credential_alias`: a routing hint (an absent or stale jar leads to a jarless session and a login
attempt rather than `login_required`) and the default alias the browser profile asks for. It
authorizes nothing.
...
every authenticated-site session, `browser_autofill` is available in any of them, and a jarless
session (confinement origins supplied explicitly from `authenticated_origins`, validated by
browser-server, confined identically to a jar-loaded one) can be created for any site whose entry
sets `credential_alias`. Nothing is bound at session creation, so there is no fill-time transition
and no binding to validate at startup; a site whose alias has no policy row simply gets
...
model-visible data, and a household with two accounts on one site disambiguates by alias exactly as
it does by site id. What the model cannot do is obtain a release that neither a policy row nor a
human approved, browse the Keychute store, or fill outside the granted origins — Keychute and
browser-server enforce that, not identifier hiding.
...
- `refused(reason)`: wrong origin, no eligible field, target invalidated by navigation, policy
  denial, unknown alias, or a bad-password outcome already recorded in this session.
...
1. **browser-server:** Keychute client (in-cluster URL, internal CA); explicit confinement origins
   on jarless `create_session`; the autofill endpoint with requests tied to the checked document's
   origin, granted-constraint destination checks, serialized target binding, element checks, one
   request + one read per fill step; uniform read-back protection in authenticated-site sessions.
   Regression tests: plaintext in no response/event/log/exception; fills refused off the granted
   origin, in iframes, on new-password fields, after target invalidation, and outside
   authenticated-site sessions; masking, clipboard denial, and `exec` denial active before the first
   fill; a jarless session confined identically to a jar-loaded one.
2. **family-assistant:** `credential_alias` on `authenticated_sites`; the `browser_autofill` tool in
   the authenticated profiles; `approval_pending` as a parked resumable outcome; stale-jar runs for
   alias-bearing sites routed to the jarless session; docs.
