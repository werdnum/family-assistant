...
6. **Privacy policy URL** — App Store Connect requires one before a build can be distributed. The
   backend serves the policy unauthenticated at `/privacy`, so enter
   `https://<your SERVER_URL>/privacy` under App Information → Privacy Policy URL. Set
   `PRIVACY_POLICY_OPERATOR` and `PRIVACY_POLICY_CONTACT_EMAIL` on the server first (see
