...
# "Tell me when Alex gets home"
{
    ...
    "action_config": {
        "prompt": "Alex just arrived home. Send a notification to the user.",
        "context": {"person": "Alex"}
    },
...
[calls test_event_listener(
    source_id="home_assistant",
    match_conditions={"entity_id": "person.alex", "new_state.state": "home"},
    hours=24
)]

I see no events matched with lowercase 'home'. Let me check what events exist for Alex...
[calls query_recent_events(
    source_id="home_assistant",
    hours=24
) and filters for person.alex]

I found that person.alex uses states 'Home' and 'Away' (capitalized). Let me test again with the correct state value:
[calls test_event_listener(
    source_id="home_assistant",
    match_conditions={"entity_id": "person.alex", "new_state.state": "Home"},
    hours=24
...
Created listener #1. I tested it against recent events and confirmed it will trigger when Alex's state changes to 'Home'.
...
- **Entity names vary**: `person.alex` vs `person.alex_smith` vs `device_tracker.alexs_iphone`
